...
export default function TinyWorld() {
  ...
  const revealRef = useRef<{ start: number; dur: number; depth: number; active: boolean } | null>(null);
  const zoneRaisesRef = useRef<number[]>([]);
  ...
  // ─── Fullscreen toggle (video-player style) ──────────────────────────────
  ...
  const toggleFullscreen = useCallback(async () => {
    ...
    } catch { /* iframe without allow="fullscreen" — CSS path already applied */ }
  }, []);
  ...
  // Web Speech API — voice command goal-injection for villagers. Each
  ...
  useEffect(() => {
    ...
    if (params.get("progressive") === "1") return;
  ...
  const buildWorld = useCallback(async (source: File | { layers: Record<string, Int32Array>; meta: Record<string, unknown>; blockCount: number; resolution: number }) => {
    ...
    // ── Real-world solar geometry. NOAA-approximation sunrise/solar-noon/sunset
    ...
    // it — TOD now respects season + latitude.
    ...
    // Map real epoch ms → virtual TOD hour: solar midnight → 0/24, sunrise → 6,
    // solar noon → 12, sunset → 18. Piecewise-linear so the palette lerp lands
    ...
    // alt/az. Altitude is clamped to a minimum 8° above horizon so shadows
    // keep casting from above even when the sun has set — at night the TOD
    ...
    // aware) so its texels line up 1:1 with the composer's diffuse target —
    ...
    // distribution over a 0.02→500 frustum left almost no precision past ~50
    ...
    // Default OFF — user A/B'd the band-diagnostic panel and prefers the
    ...
    // 2048² shadow map's texel density high so each voxel face gets a few
    ...
    // Image-based lighting (PMREM) disabled by default — the env-map horizon
    ...
    // ── Magnesis-style pick FX ────────────────────────────────────────────
    // Pool of glowing proxy cubes. On harvest a block detaches and magnetically
    // lifts, hovers, then streaks into the lower-screen stockpile and dissolves.
    const MAGNESIS_YELLOW = new THREE.Color(0xffe66a);
    const pickFxGeo = new THREE.BoxGeometry(voxel * 0.94, voxel * 0.94, voxel * 0.94);
    const pickFxEdgeGeo = new THREE.EdgesGeometry(pickFxGeo);
    const pickFxPool: Array<any> = [];
    for (let pf = 0; pf < 10; pf++) {
      const fxMat = new THREE.MeshBasicMaterial({ color: 0xffffff, transparent: true, opacity: 0, depthWrite: false });
      const fxMesh = new THREE.Mesh(pickFxGeo, fxMat);
      fxMesh.visible = false;
      fxMesh.renderOrder = 6;
      const glowMat = new THREE.LineBasicMaterial({ color: 0xffe66a, transparent: true, opacity: 0, blending: THREE.AdditiveBlending, depthWrite: false });
      const glow = new THREE.LineSegments(pickFxEdgeGeo, glowMat);
      fxMesh.add(glow);
      scene.add(fxMesh);
      pickFxPool.push({ mesh: fxMesh, mat: fxMat, glow, glowMat, active: false });
    }
    const pickFx: Array<any> = [];
    const PICK_FX_MS = 520;
    const spawnPickFx = (wx: number, wy: number, wz: number, color: any, size: number = 1) => {
      const slot = pickFxPool.find((p) => !p.active);
      if (!slot) return;
      slot.active = true;
      slot.mesh.visible = true;
      slot.mesh.position.set(wx, wy, wz);
      slot.mesh.scale.setScalar(size);
      slot.mesh.rotation.set(0, 0, 0);
      if (color) slot.mat.color.copy(color); else slot.mat.color.setHex(0xffffff);
      slot.mat.opacity = 1;
      slot.glowMat.opacity = 0.9;
      pickFx.push({ slot, startMs: performance.now(), x0: wx, y0: wy, z0: wz, size });
    };
    // Magnesis lift-and-fade: each picked block hovers up a touch, turns gently,
    // and dissolves in place — no streak into a lower-screen stockpile bin.
    const tickPickFx = (now: number) => {
      for (let i = pickFx.length - 1; i >= 0; i--) {
        const fx = pickFx[i];
        const t = (now - fx.startMs) / PICK_FX_MS;
        if (t >= 1) {
          fx.slot.active = false;
          fx.slot.mesh.visible = false;
          fx.slot.mat.opacity = 0;
          fx.slot.glowMat.opacity = 0;
          pickFx.splice(i, 1);
          continue;
        }
        const ease = 1 - Math.pow(1 - t, 2);
        fx.slot.mesh.position.set(fx.x0, fx.y0 + voxel * 0.6 * ease * fx.size, fx.z0);
        fx.slot.mesh.rotation.y = ease * 1.2;
        fx.slot.mesh.rotation.x = ease * 0.6;
        fx.slot.mat.opacity = 1 - t;
        fx.slot.glowMat.opacity = 0.9 * (1 - t);
      }
    };
    ...
    let voidVig = document.getElementById("void-vignette");
    if (!voidVig) {
      voidVig = document.createElement("div");
      voidVig.id = "void-vignette";
      Object.assign(voidVig.style, {
        position: "fixed",
        inset: "0",
        pointerEvents: "none",
        opacity: "0",
        background:
          "radial-gradient(ellipse at center, rgba(0,0,0,0) 32%, rgba(10,4,26,0.55) 76%, rgba(4,1,12,0.94) 100%)",
        boxShadow: "inset 0 0 240px 70px rgba(48,14,84,0.55)",
        transition: "opacity 0.12s linear",
        zIndex: "9998"
      });
      document.body.appendChild(voidVig);
    }
    ...
    // ── Tectonic reveal ──────────────────────────────────────────────────
    // Sink every terrain layer below the void, then raise them over the next
    // ~1.5s so a freshly loaded world rises into place instead of popping in.
    // Cosmetic only (mesh.position.y); physics keys off groundRef, so collision
    // is untouched. start:0 → lazy-inits to the loop clock on the first frame,
    // which keeps the full animation even if async build work delays the loop.
    {
      const revDepth = 18 * voxel;
      for (const m of meshesRef.current) m.position.y = -revDepth;
      revealRef.current = { start: 0, dur: 1500, depth: revDepth, active: true };
    }
    ...
    // ~sentinel eye-level (sentinel ≈ 13.5v tall, eyes ≈ 11.5v above feet,
    // so lift ≈ 11.5 − EYE_HEIGHT). Sentinel feet still anchor to the
    ...
    const buildWorkerVisual = (style: "humanoid" | "tars" | "ghibli") => {
      ...
      let carried: any;
      if (style === "ghibli") {
        // Chibi voxel character (~2 voxels tall) matching the reference
        // image: big head, small puffy jacket body, dark skirt, red socks,
        // pink shoes, brown bob hair. Proportions are deliberately chibi —
        // the head is the largest single section so the silhouette reads as
        // a cute villager even when zoomed out. Eyes are dark (not glowing
        // red) so the character doesn't look demonic at night.
        ...
        // Dark eyes — faint emissive keeps them visible at night without glow.
        const eyeMat = new THREE.MeshStandardMaterial({ color: 0xffffff, emissive: 0x2a201a, emissiveIntensity: 0.4, roughness: 1, metalness: 0 });
        const cubeGeo = new THREE.BoxGeometry(u, u, u);

        // ── Palette (matched to reference) ──
        const SKIN       = 0xffe9d6;
        const SKIN_SHADE = 0xe8c6a8;
        const SHIRT      = 0xf4f4f4;
        const J_WHITE    = 0xe6e6e6;
        const J_RED      = 0xd96a52;
        const J_BLUE     = 0x4f86c4;
        const SKIRT      = 0x2b2b2b;
        const SOCKS      = 0xb84a4a;
        const SHOES      = 0xe299aa;
        const HAIR       = 0x4a3930;
        const HAIR_HI    = 0x5a4840;
        const LIP        = 0xc97766;
        ...
        // ── BODY (sub-voxel coords; y=0 = feet bottom, +z = forward) ──
        // Chibi stack: short legs → skirt → small torso → big head.
        // Skirt (hip block)
        fill(rootC, SKIRT, -4, 5, 11, 15, -3, 4);
        // Torso / shirt (small)
        fill(rootC, SHIRT, -3, 4, 15, 21, -2, 3);
        // Jacket sides (white)
        fill(rootC, J_WHITE, -4, -3, 15, 21, -3, 4);
        fill(rootC, J_WHITE,  4,  5, 15, 21, -3, 4);
        // Jacket back
        fill(rootC, J_WHITE, -3, 4, 15, 21, -3, -2);
        // Patterned panels (orange left, blue right) on the chest
        fill(rootC, J_RED,  -3, -1, 17, 20, 3, 4);
        fill(rootC, J_BLUE,  1,  4, 17, 20, 3, 4);
        // Collar / hood (white)
        fill(rootC, J_WHITE, -4, 5, 20, 21, -3, 4);
        // Neck (skin)
        fill(rootC, SKIN, -1, 2, 21, 22, -1, 2);

        // Head — BIG chibi head (10w × 9h × 8d), the largest single section
        fill(rootC, SKIN, -5, 5, 22, 31, -3, 5);
        // Cheek/jaw shading on lower head sides
        fill(rootC, SKIN_SHADE, -5, 5, 22, 24, 4, 5);
        // Small lip dot
        fill(rootC, LIP, 0, 1, 24, 25, 5, 6);

        // ── HAIR (layered bob cut framing the big head) ──
        // Top cap
        fill(rootC, HAIR, -6, 7, 30, 34, -4, 5);
        fill(rootC, HAIR_HI, -4, 5, 32, 34, -2, 3);
        // Back wall behind head
        fill(rootC, HAIR, -6, 7, 22, 30, -4, -3);
        // Side walls outside head
        fill(rootC, HAIR, -7, -5, 23, 30, -3, 4);
        fill(rootC, HAIR,  5,  7, 23, 30, -3, 4);
        // Bangs (front cap above eyes)
        fill(rootC, HAIR, -5, 6, 28, 31, 5, 6);
        // Side-swept bang locks hanging past the eyes
        fill(rootC, HAIR, -6, -4, 24, 28, 5, 6);
        fill(rootC, HAIR,  4,  6, 24, 28, 5, 6);

        // ── EYES (dark, on the big face) ──
        fill(eyeC, 0x231a14, -3, -1, 25, 27, 5, 6);
        fill(eyeC, 0x231a14,  1,  3, 25, 27, 5, 6);

        // ── ARMS (pivots at shoulders y≈20; cubes in pivot-local coords) ──
        // Jacket shoulder (red)
        ...
        // ── LEGS (pivots at hips y≈11; cubes in pivot-local coords) ──
        // Upper leg (bare skin, short)
        fill(legLC, SKIN, -1, 2, -2, 0, -1, 2);
        fill(legRC, SKIN, -1, 2, -2, 0, -1, 2);
        // High socks (red)
        fill(legLC, SOCKS, -1, 2, -8, -2, -1, 2);
        fill(legRC, SOCKS, -1, 2, -8, -2, -1, 2);
        // Shoes (pink, chunky — wider than the sock)
        fill(legLC, SHOES, -2, 3, -11, -8, -2, 3);
        fill(legRC, SHOES, -2, 3, -11, -8, -2, 3);
        // Shoe toe
        fill(legLC, 0xdd7788, -2, 3, -11, -10, 2, 4);
        fill(legRC, 0xdd7788, -2, 3, -11, -10, 2, 4);
        ...
        // Shoulder at torso-top (rootC y=20), hip at skirt-bottom (rootC y=11).
        const armPivotL = new THREE.Group();
        const armPivotR = new THREE.Group();
        armPivotL.position.set(-5 * u, feetY + 20 * u, 0);
        armPivotR.position.set( 5 * u, feetY + 20 * u, 0);
        group.add(armPivotL);
        group.add(armPivotR);
        ...
        legPivotL.position.set(-2 * u, feetY + 11 * u, 0);
        legPivotR.position.set( 2 * u, feetY + 11 * u, 0);
        group.add(legPivotL);
        group.add(legPivotR);
        addInst(legPivotL, legLC, bodyMat);
        addInst(legPivotR, legRC, bodyMat);

        // Carried block (held above the head)
        carried = new THREE.Mesh(
          new THREE.BoxGeometry(voxel * 0.5, voxel * 0.5, voxel * 0.5),
          new THREE.MeshPhongMaterial({ color: 0xffffff }),
        );
        carried.position.set(0, feetY + voxel * 2.9, 0);
    ...
    const tickWorkers = (now: number) => {
      for (const w of workers) {
        ...
        // and findAdjacentWalkableW all use this offset — there is NO +1).
        if (w.mode === "idle" && !w.pickupTarget && !w.placeTarget && !(w as any).buildTarget) {
          ...
          if (top !== null) {
            ...
            // Column under worker is empty — try a neighbor first.
            let stepped = false;
            ...
            if (!stepped) {
              // Build escape route — spawn a stockpile block at the
        ...
        if (w.group.userData.style === "ghibli") {
          ...
          if (isWalking) {
            ...
            if (gait === "slow") {
              ...
              // Walk modulation 2: A-shape split — outer slabs tilt forward,
              ...
              // Inner slabs tilt opposite each other at ~1.6× the frequency,
    ...
    const completePickup = () => {
      ...
      const isLargeSentinel = sentinelModeRef.current === "large";
      if (isLargeSentinel) {
        // One big 4x4x4 Magnesis cube centered on the swept volume
        // (sweep offsets run -1..+2, so its center sits at +0.5 each axis).
        spawnPickFx((vx + 0.5 - cxRound) * voxel, (vy + 0.5) * voxel, (vz + 0.5 - czRound) * voxel, color, 4);
      } else {
    ...
    // Sentinel LARGE mode: 4x4x4 bulk block ops. Sweeps a cube centered on the
    // targeted block (offsets -1..+2 each axis), pulling every solid cell into
    // stockpile via the ledger. Skips water + any unmoveable layer. The single
    // block the user originally picked stays in carry; this adds bulk dividend.
    ...
    // Bulk place: fills the 4x4x4 cube centered on (cx,cy,cz) with blocks
    // drawn from stockpile. Prefers the carry layer; falls back to dirt/stone
    // for any cell whose preferred layer is exhausted. Respects ledger.
    ...
    (window as any).__tw = {
      pickFxTest: (size: number = 1) => {
        const d = new THREE.Vector3();
        camera.getWorldDirection(d);
        const wx = camera.position.x + d.x * voxel * 6;
        const wy = camera.position.y + d.y * voxel * 6;
        const wz = camera.position.z + d.z * voxel * 6;
        spawnPickFx(wx, wy, wz, MAGNESIS_YELLOW, size);
        return { spawned: true, size, at: { wx, wy, wz } };
      },
      addZone: (payload: { layers?: Record<string, string>; origin?: number[] }) => {
        if (!payload || !payload.layers) return { ok: false, added: 0 };
        const ZS: Record<string, [number, number]> = { dirt: [PAL.dirt, 1], grass: [PAL.grass, 1], dryGrass: [PAL.dryGrass, 1], snow: [PAL.snow, 1], leaves: [PAL.leaf, 0.95], stone: [0x7c7c84, 1], metal: [0xb8bcc4, 1], fruit: [PAL.fruit, 1] };
        const dec = (b64: string): Int32Array => { const bin = atob(b64); const u8 = new Uint8Array(bin.length); for (let i = 0; i < bin.length; i++) u8[i] = bin.charCodeAt(i); return new Int32Array(u8.buffer, 0, Math.floor(u8.byteLength / 4)); };
        const newMeshes: any[] = []; let added = 0;
        for (const layerName of Object.keys(payload.layers)) {
          const b64 = payload.layers[layerName];
          if (typeof b64 !== "string" || !b64) continue;
          const arr = dec(b64); if (!arr.length) continue;
          const style = ZS[layerName] || [PAL.dirt, 1];
          const mesh = addLayer(arr, style[0], style[1], layerName);
          if (mesh) { mesh.userData.zoneMesh = true; newMeshes.push(mesh); added += Math.floor(arr.length / 3); }
        }
        if (newMeshes.length === 0) return { ok: false, added: 0 };
        for (const [k, cs] of colMap) { if (!cs || cs.size === 0) continue; let top = -Infinity; for (const y of cs) if (y > top) top = y; groundMap.set(k, top); }
        setBlocks(currentBlocks);
        const depth = 18 * voxel; const dur = 1400;
        for (const m of newMeshes) m.position.y = -depth;
        let zstart = 0;
        const step = (now: number) => {
          if (zstart === 0) zstart = now;
          const t = Math.min(1, (now - zstart) / dur);
          const ease = 1 - Math.pow(1 - t, 3);
          for (const m of newMeshes) m.position.y = -depth * (1 - ease);
          if (t < 1) { zoneRaisesRef.current.push(requestAnimationFrame(step)); } else { for (const m of newMeshes) m.position.y = 0; }
        };
        zoneRaisesRef.current.push(requestAnimationFrame(step));
        return { ok: true, added, meshes: newMeshes.length };
      },
      replayReveal: () => {
        const revDepth = 18 * voxel;
        for (const m of meshesRef.current) m.position.y = -revDepth;
        revealRef.current = { start: 0, dur: 1500, depth: revDepth, active: true };
        return { layers: meshesRef.current.length, depth: revDepth };
      },
      // Fit the orbit camera to the accumulated terrain. Used by the progressive
      // loader to zoom out as new zones rise. Frames on the horizontal footprint
      // (X/Z) and aims at box.max.y — the highest *settled* surface — so a zone
      // mid-rise (sunk to -depth) extends box.min.y without dragging the aim down.
      frameWorld: () => {
        const meshes = meshesRef.current;
        if (!meshes || meshes.length === 0) return { ok: false };
        ...
        for (const m of meshes) {
          ...
          } catch { /* skip degenerate mesh */ }
        }
        if (!any || box.isEmpty()) return { ok: false };
        const center = box.getCenter(new THREE.Vector3());
        const size = box.getSize(new THREE.Vector3());
        const footprint = Math.max(size.x, size.z, voxel * 4);
        const dist = footprint * 0.95 + voxel * 8;
        const ty = box.max.y;
        orbit.target.set(center.x, ty, center.z);
        camera.position.set(center.x + dist, ty + dist * 0.7, center.z + dist);
        camera.lookAt(center.x, ty, center.z);
        orbit.update();
        return { ok: true, center: [center.x, ty, center.z], footprint };
      },
      state: () => ({
        voxel,
        locked: fp.isLocked,
      ...
      diagBand: (opts: any = {}) => {
        ...
        if (opts.fog === true)  { scene.fog = savedFog; (scene as any).__diagSavedEnv = null; }
        ...
        // Kill the entire postprocess chain in one go — renders raw scene
    ...
    // ─── Supplier Sentinel: MechGolem GLB w/ rigged animations ────────────────
    // Loaded from /sentinel.glb — 13 joints, 4 baked clips
    // (Sitting, SitToStand, StandToSit, WalkSlow). Sentinel sits dormant
    // when the user is in the void; on Walk mode (F) entry it plays
    // SitToStand → WalkSlow (gentle standing sway). On exit, StandToSit →
    ...
    // Face the world centroid — flip 180° so the eye cluster faces inward
    ...
    const tickSentinel = (now: number) => {
      ...
      if (sentinelAnimState === "standing") {
        ...
        if (walkAction) {
          ...
          // Slow the gait: even at full pace, WalkSlow runs at 0.4× the
    ...
    const renderSentinel = (_now: number) => {
      ...
      // facing — plus the default 180° flip so the sentinel's eyes look
      ...
      // Out of walk mode (void/orbit view): anchor to the ground cell
      // NEAREST the world centroid on first render. The centroid itself is
      // an average of all cells and is almost never a real ground cell, so
      // reading groundMap.get(centroid) returned undefined and dropped the
      // sentinel to vy=1 — sitting at the bottom of the world. Search for
      // the nearest cell with clear headroom above it so the resting
      // sentinel isn't buried under a ceiling/overhang either.
      if (groundMap.size > 0 && sentinelState.vx === sx) {
        ...
        const cx = Math.round(sumX / n), cz = Math.round(sumZ / n);
        let best: { vx: number; vz: number; top: number; d2: number } | null = null;
        for (const [k, top] of groundMap) {
          const [vx, vz] = k.split(",").map(Number);
          const col = colMap.get(k);
          if (col) {
            let blocked = false;
            for (let dy = 1; dy <= 14; dy++) if (col.has(top + dy)) { blocked = true; break; }
            if (blocked) continue;
          }
          const d2 = (vx - cx) * (vx - cx) + (vz - cz) * (vz - cz);
          if (!best || d2 < best.d2) best = { vx, vz, top, d2 };
        }
        if (best) {
          sentinelState.vx = best.vx;
          sentinelState.vz = best.vz;
          sentinelState.vy = best.top + 1;
        } else {
          const ct = groundMap.get(cx + "," + cz);
          sentinelState.vx = cx;
          sentinelState.vz = cz;
          sentinelState.vy = (ct !== undefined ? ct : 0) + 1;
        }
        console.log("[tinyworld] sentinel anchored at", sentinelState.vx, sentinelState.vy, sentinelState.vz, "(nearest clear cell to centroid)");
      }
    ...
    const loop = () => {
      try {
        ...
        if (frameCount % 120 === 0 && !_todOv) {
          ...
          // hemi.intensity intentionally NOT updated — hemi is OFF by default
          ...
          // position right now. Updating position alone is enough — the
        ...
        if (revealRef.current && revealRef.current.active) {
          ...
          if (rv.start === 0) rv.start = now;
          const t = Math.min(1, (now - rv.start) / rv.dur);
          const ms = meshesRef.current;
          for (let mi = 0; mi < ms.length; mi++) {
            if (ms[mi].userData && ms[mi].userData.zoneMesh) continue;
            const stag = Math.min(0.28, mi * 0.045);
            const lt = (t - stag) / (1 - stag);
            const ct = lt <= 0 ? 0 : lt >= 1 ? 1 : lt;
            const ease = 1 - Math.pow(1 - ct, 3);
            ms[mi].position.y = -rv.depth * (1 - ease);
          }
          if (t >= 1) {
            for (const m of ms) { if (m.userData && m.userData.zoneMesh) continue; m.position.y = 0; }
            rv.active = false;
          }
        }
        ...
        const wasStuck = !isDrone && bodyBlockedAtY(camera.position.x, camera.position.y, camera.position.z);

        let inF = 0, inR = 0;
        if (isMobileRef.current) {
          const j = joystickRef.current;
          if (j && j.active) { inF = -j.y; inR = j.x; }
        } else {
          const k = keysRef.current;
        ...
        if (inLen > 0) {
          ...
          const moveSpeed = isDrone ? speed * 1.8 : speed;
          camera.position.addScaledVector(moveDir, moveSpeed);
        }

        // Drone vertical thrust: hold ascend/descend (mobile) or Space/Shift
        // (desktop). Unbounded - unlimited flight by design.
        if (isDrone) {
          ...
          // Ambient hover bob: tiny sinusoid so the drone feels alive even
          // with zero input.
        ...
        const footWY = camera.position.y - EYE_HEIGHT;
        const eye = sampleGround(camera.position.x, camera.position.z, footWY, playerR) + EYE_HEIGHT;
        let overWorld = false;
        {
          const gnd = groundRef.current;
          if (gnd && gnd.map.size > 0) {
            const v = gnd.voxel;
            const minVX = Math.round((camera.position.x - playerR) / v) + gnd.cx;
            const maxVX = Math.round((camera.position.x + playerR) / v) + gnd.cx;
            const minVZ = Math.round((camera.position.z - playerR) / v) + gnd.cz;
            const maxVZ = Math.round((camera.position.z + playerR) / v) + gnd.cz;
            for (let vx = minVX; vx <= maxVX && !overWorld; vx++)
              for (let vz = minVZ; vz <= maxVZ && !overWorld; vz++)
                if (gnd.map.has(vx + "," + vz)) overWorld = true;
            if (!overWorld) {
              const cvx = Math.round(camera.position.x / v) + gnd.cx;
              const cvz = Math.round(camera.position.z / v) + gnd.cz;
              for (let dx = -1; dx <= 1 && !overWorld; dx++)
                for (let dz = -1; dz <= 1 && !overWorld; dz++)
                  if (gnd.map.has((cvx + dx) + "," + (cvz + dz))) overWorld = true;
            }
          }
        }

        if (walkingRef.current && !isDrone && !recoveringFromStuck) {
          if (!overWorld) {
        ...
        if (voidVig) {
          let vigTarget = 0;
          if (!isDrone && !overWorld) {
            const gv = groundRef.current;
            const span = gv ? Math.max(1, 40 * gv.voxel) : 40;
            const groundY = gv ? gv.minY * gv.voxel : camera.position.y;
            const depth = Math.max(0, groundY - camera.position.y);
            vigTarget = Math.min(0.9, 0.12 + 0.78 * (depth / span));
          }
          voidVig.style.opacity = String(vigTarget);
        }
        ...
        if (composerRef.current) {
          if ((window as any).__twOutline?.state?.enabled) {
          ...
          outlinePass.enabled = (window as any).__twOutline?.state?.enabled;
  ...
  const onLoadSelected = useCallback(async (overrideId?: string) => {
    ...
    try {
      ...
      // Saved worlds (esp. progressive scans) store span as a raw voxel-count and
      // centerX/Z=0, so buildWorld frames the orbit camera tens of metres off in a
      // corner -> "blank world on load". Reframe onto the actual geometry once it
      // is built, then again after the tectonic reveal settles.
      requestAnimationFrame(() => { try { (window as any).__tw?.frameWorld?.(); } catch {} });
      setTimeout(() => { try { (window as any).__tw?.frameWorld?.(); } catch {} }, 1700);
  ...
  // ─── Progressive world load ───────────────────────────────────────────────
  // Streams a still-being-inferred world: polls the infer endpoint by capture
  // id, builds the scene from zone 0 (the first small island, with the hero
  // tectonic reveal), then feeds each subsequent zone into __tw.addZone so new
  // land rises into place as the server keeps producing geometry. Opt-in via
  // /tinyworld?progressive=1&capture=<id>; the standard loaders bail on that flag.
  const loadWorldProgressive = useCallback(async (captureId: string) => {
    const INF = `/api/tinyworld-infer?capture_id=${encodeURIComponent(captureId)}`;
    const sleep = (ms: number) => new Promise((r) => setTimeout(r, ms));
    const consumed = new Set<number>();
    let built = false;
    const deadline = Date.now() + 6 * 60 * 1000;
    const fetchZone = async (idx: number) => {
      try {
        const r = await fetch(`${INF}&zone=${idx}`, { headers: { Accept: "application/json" } });
        if (!r.ok) return null;
        const j = await r.json();
        return j?.ok ? j.zone : null;
      } catch { return null; }
    };
    const VOX = 0.08;
    const metaOf = (ls: Record<string, Int32Array>) => {
      let mnX = Infinity, mxX = -Infinity, mnZ = Infinity, mxZ = -Infinity;
      for (const a of Object.values(ls)) {
      ...
      if (!isFinite(mnX)) return { span: 10, centerX: 0, centerZ: 0 };
      // span/center in METRES: buildWorld treats meta.span as metres for camera
      // framing and recentres blocks by round(centerX/voxel), so passing raw
      // voxel counts (and centerX:0) left the island ~50 m off in the corner.
      return {
        span: Math.max(mxX - mnX, mxZ - mnZ) * VOX,
        centerX: ((mnX + mxX) / 2) * VOX,
        centerZ: ((mnZ + mxZ) / 2) * VOX,
      };
    };
    while (Date.now() < deadline) {
      let meta: any = null;
      try {
        const r = await fetch(INF, { headers: { Accept: "application/json" } });
        meta = await r.json();
      } catch { await sleep(900); continue; }
      if (!meta?.ok) { await sleep(900); continue; }
      const zoneList: any[] = Array.isArray(meta.zones) ? meta.zones : [];
      if (!built && zoneList.length > 0) {
        const z0 = await fetchZone(zoneList[0].zone_index);
        if (z0?.payload?.layers) {
          const layers = layersFromPayload({ layers: z0.payload.layers } as any) as Record<string, Int32Array>;
          const m0 = metaOf(layers);
          await buildWorld({
            layers,
            meta: { voxel: 0.08, span: m0.span, centerX: m0.centerX, centerZ: m0.centerZ, weatherSeason: "summer", progressive: true, worldName: "TinyWorld (growing…)" },
            blockCount: z0.block_count || 0,
            resolution: 0.08,
          } as any);
          built = true;
          consumed.add(zoneList[0].zone_index);
          // Recover layers buildWorld doesn't render natively (stone/metal/fruit).
          const extra: Record<string, string> = {};
          for (const k of ["stone", "metal", "fruit"]) {
            const v = z0.payload.layers[k];
            if (v) extra[k] = v;
          }
          if (Object.keys(extra).length > 0) (window as any).__tw?.addZone?.({ layers: extra });
          setLoadNote("island forming — more land rising…");
        }
      }
      if (built) {
        let grew = false;
        for (const zm of zoneList) {
          if (consumed.has(zm.zone_index)) continue;
          const z = await fetchZone(zm.zone_index);
          if (z?.payload?.layers) {
            (window as any).__tw?.addZone?.(z.payload);
            consumed.add(zm.zone_index);
            grew = true;
            setLoadNote(`risen ${consumed.size} zone${consumed.size === 1 ? "" : "s"}…`);
          }
        }
        if (grew) (window as any).__tw?.frameWorld?.();
      }
      if (meta.status === "processed" && built && consumed.size >= zoneList.length) {
        setLoadNote(`world complete — ${meta.blockCount?.toLocaleString?.() ?? ""} blocks`);
        break;
      }
      if (meta.status === "failed") { setLoadNote(`inference failed: ${meta.error || ""}`); break; }
      await sleep(1100);
    }
  }, [buildWorld]);
  ...
  useEffect(() => {
    ...
    if (sp.get("progressive") !== "1") return;
    const cap = sp.get("capture");
    if (!cap) return;
    if (autoLoadRef.current) return;
    autoLoadRef.current = true;
    setLoadNote("connecting to live build…");
    loadWorldProgressive(cap).catch((e) => setLoadNote(`progressive load failed: ${e?.message || e}`));
  }, [loadWorldProgressive]);
    ...
    >
      ...
      {phase === "idle" && (
        <div style={ui.center}>
          ...
          <a
            href="/tinyworld/capture?progressive=1"
            style={{
              display: "inline-flex",
              alignItems: "center",
              gap: 10,
              marginTop: 8,
              padding: "16px 30px",
              background: "linear-gradient(180deg, rgba(138,43,226,0.40), rgba(138,43,226,0.16))",
              ...
              cursor: "pointer",
              boxShadow: "0 8px 32px rgba(138,43,226,0.35)",
            }}
          >
          ...
          <p style={{ ...ui.sub, opacity: 0.55, marginTop: 4 }}>
            walk through your room · builds a tiny world from what you see
          </p>
          <label
            style={{
              display: "inline-flex",
          ...
          <div className="absolute bottom-8 left-8 flex flex-col gap-6">
            ...
            <div className="flex flex-col gap-3">
              ...
              <button
                className="group flex items-center gap-3"
                onClick={toggleVoice}
                title={voiceError ? "Voice: " + voiceError : (voiceListening ? "Listening — speak a goal" : "Voice command")}>
            ...
            <div className="pointer-events-none absolute left-6 bottom-24 z-30 max-w-md">
              <div className="px-3 py-2 rounded-md border border-amber-300/30 bg-black/60 backdrop-blur-sm">
                <div className="text-[9px] text-amber-200/70 uppercase tracking-widest font-mono">
                  {voiceListening ? "voice command → villager goal" : "last command"}
                </div>
                {voiceTranscript && (
