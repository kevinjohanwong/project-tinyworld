...
export default function TinyWorld() {
  ...
  const buildWorld = useCallback(async (source: File | { layers: Record<string, Int32Array>; meta: Record<string, unknown>; blockCount: number; resolution: number }) => {
    ...
    const droneThrusterMat = new THREE.MeshStandardMaterial({ color: 0x67e8f9, emissive: 0x67e8f9, emissiveIntensity: 1.3, roughness: 0.3, metalness: 0 });
    ...
    const droneLight = new THREE.PointLight(0x67e8f9, 0.5, voxel * 14, 2);
    ...
    (window as any).__tw = {
      ...
      sentinelInfo: () => ({
        ...
        thirdPerson: { enabled: true, forward: thirdPersonRig.sentinelForward },
    ...
    const renderSentinel = (_now: number) => {
      ...
      if (walkingRef.current) {
        ...
        const bodyGroundY = sampleGround(bodyX, bodyZ, camera.position.y - EYE_HEIGHT - WALK_CAMERA_LIFT, playerR);
      ...
      try {
        ...
        if (!simFrozen && (fp.isLocked || __twSim.walk || (isMobileRef.current && walkingRef.current))) {
          ...
          if (isDrone) {
            ...
            const bFootWy = camera.position.y - EYE_HEIGHT - WALK_CAMERA_LIFT;
          ...
          const camGroundY = sampleGround(camera.position.x, camera.position.z, camera.position.y - EYE_HEIGHT - WALK_CAMERA_LIFT, playerR);
          ...
          const eye = eyeGroundY + EYE_HEIGHT + WALK_CAMERA_LIFT;
