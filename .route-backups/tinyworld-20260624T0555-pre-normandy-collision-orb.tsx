...
export default function TinyWorld() {
  ...
  const buildWorld = useCallback(async (source: File | { layers: Record<string, Int32Array>; meta: Record<string, unknown>; blockCount: number; resolution: number }) => {
    ...
    const droneCore = new THREE.Mesh(
      new THREE.OctahedronGeometry(voxel * 0.35, 1),
      new THREE.MeshBasicMaterial({ color: 0x67e8f9, transparent: true, opacity: 0.9, depthTest: false })
    );
    const droneHalo = new THREE.Mesh(
      new THREE.RingGeometry(voxel * 0.55, voxel * 0.76, 24),
      new THREE.MeshBasicMaterial({ color: 0xa5f3fc, transparent: true, opacity: 0.55, depthTest: false, side: THREE.DoubleSide })
    );
    const droneWingMat = new THREE.MeshBasicMaterial({ color: 0x0f172a, transparent: true, opacity: 0.82, depthTest: false });
    const droneWingGeo = new THREE.BoxGeometry(voxel * 1.1, voxel * 0.08, voxel * 0.16);
    const droneWingL = new THREE.Mesh(droneWingGeo, droneWingMat);
    const droneWingR = new THREE.Mesh(droneWingGeo, droneWingMat);
    droneWingL.position.x = -voxel * 0.55;
    droneWingR.position.x = voxel * 0.55;
    const droneLight = new THREE.PointLight(0x67e8f9, 0.45, voxel * 16, 2);
    droneRig.add(droneHalo, droneCore, droneWingL, droneWingR, droneLight);
    scene.add(droneRig);
    let droneRigVisible = false;
    const thirdPersonRig = {
      sentinelForward: voxel * 26,
    ...
    const tickOrb = (now: number) => {
      ...
      orbRef.prev = now;
      // Companion anchor: above-left of the camera, in view.
      const fwd = new THREE.Vector3();
      camera.getWorldDirection(fwd);
      const right = new THREE.Vector3().crossVectors(fwd, camera.up).normalize();
      const anchor = camera.position.clone()
        .add(fwd.clone().multiplyScalar(voxel * 3.2))
        .add(right.clone().multiplyScalar(-voxel * 2.0))
        .add(new THREE.Vector3(0, voxel * 1.6, 0));
      orbRef.target.copy(anchor);
      orbRef.pos.lerp(orbRef.target, Math.min(1, dt * 4.5));
      // Bob + pulse + billboard halo.
      orbRef.bobPhase += dt * 2.2;
      const bob = Math.sin(orbRef.bobPhase) * voxel * 0.25;
