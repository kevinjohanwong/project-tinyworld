...
export default function TinyWorld() {
  ...
  const loadWorldProgressive = useCallback(async (captureId: string) => {
    ...
    const spanOf = (ls: Record<string, Int32Array>) => {
      ...
      return isFinite(mnX) ? Math.max(mxX - mnX, mxZ - mnZ) : 10;
    };
    while (Date.now() < deadline) {
      ...
      if (!built && zoneList.length > 0) {
        ...
        if (z0?.payload?.layers) {
          ...
          await buildWorld({
            layers,
            meta: { voxel: 0.08, span: spanOf(layers), centerX: 0, centerZ: 0, weatherSeason: "summer", progressive: true, worldName: "TinyWorld (growing…)" },
