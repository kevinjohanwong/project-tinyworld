...
export default function TinyWorld() {
  ...
  const buildWorld = useCallback(async (source: File | { layers: Record<string, Int32Array>; meta: Record<string, unknown>; blockCount: number; resolution: number }) => {
    ...
    const tickDroneRig = (now: number, dt: number, inLen: number, vert: number) => {
      ...
      droneRig.quaternion.slerp(camera.quaternion, Math.min(1, dt * 14));
      ...
      droneRig.rotation.z += (bank - droneRig.rotation.z) * Math.min(1, dt * 9);
      droneRig.rotation.x += ((vert * -0.12) - droneRig.rotation.x) * Math.min(1, dt * 8);
    ...
    type SettleFx = { mesh: any; slot: number; vx: number; vy: number; vz: number; layer: string; t0: number; shadow: any };
    type DustFx = { mesh: any; t0: number };
    type RelocationTetherFx = { line: any; t0: number };
    ...
    const takeFx = (pool: any[]) => pool.find((m) => !m.visible) || pool[0];
    ...
    const setSlotTone = (mesh: any, slot: number, vx: number, vy: number, vz: number, layer: string) => {
      ...
      const base = new THREE.Color((PAL as any)[layer] ?? (layer === "stone" ? 0x8b8f96 : layer === "metal" ? 0x9fb4c0 : 0x7b6a55));
      const h = (Math.imul(vx, 73856093) ^ Math.imul(vy, 19349663) ^ Math.imul(vz, 83492791)) >>> 0;
      base.multiplyScalar(0.94 + ((h % 1000) / 1000) * 0.07);
    ...
    const spawnDustFx = (vx: number, vy: number, vz: number) => {
      ...
      dust.visible = true;
      dustFx.push({ mesh: dust, t0: performance.now() });
    };
    const spawnRelocationTether = (src: { x: number; y: number; z: number } | null | undefined, tx: number, ty: number, tz: number) => {
      ...
      line.visible = true;
      relocationTethers.push({ line, t0: performance.now() });
    };
    const spawnSettleFx = (mesh: any, slot: number, vx: number, vy: number, vz: number, layer: string, source?: { x: number; y: number; z: number } | null) => {
      ...
      shadow.visible = true;
      settleFx.push({ mesh, slot, vx, vy, vz, layer, t0: performance.now(), shadow });
    ...
    const tickPhysicalFx = (now: number) => {
      for (let i = settleFx.length - 1; i >= 0; i--) {
        const fx = settleFx[i];
        const t = Math.min(1, (now - fx.t0) / 430);
      ...
      for (let i = dustFx.length - 1; i >= 0; i--) {
        const fx = dustFx[i];
        const t = Math.min(1, (now - fx.t0) / 520);
      ...
      for (let i = relocationTethers.length - 1; i >= 0; i--) {
        ...
        const t = Math.min(1, (now - fx.t0) / 360);
