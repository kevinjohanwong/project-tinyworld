...
// Self-contained anime.js + three-adapter bundle (one registry — proven on /anime-spike).
// Hosted as a space asset so we never `bun add` it (npm installs break on restart).
const ANIME_URL = "/lib/anime-three.bundle.mjs";
...
export default function TinyWorld() {
  ...
  // ── Orb companion / opening / tutorial / pilot UI state ──
  const [orbUi, setOrbUi] = useState<{ text: string; mood: string; visible: boolean }>({ text: "", mood: "calm", visible: false });
  const [openingUi, setOpeningUi] = useState<{ active: boolean; step: number }>({ active: false, step: 0 });
  const [pilotUi, setPilotUi] = useState<{ active: boolean; fuel: number; maxFuel: number; speed: number; altitude: number; banking: number }>({ active: false, fuel: 0, maxFuel: 8, speed: 0, altitude: 0, banking: 0 });
  const openingDoneRef = useRef(false);
  const pilotActiveRef = useRef(false);
  const orbAlertRef = useRef<(k: string, c?: any) => void>(() => {});
  const startOpeningRef = useRef<() => void>(() => {});
  const enterPilotRef = useRef<(i?: number) => void>(() => {});
  const exitPilotRef = useRef<(l?: boolean) => void>(() => {});
  ...
  // View mode: "cockpit" = first-person from the Sentinel's head (body hidden,
  // HUD frame drawn) — the default, makes control + building far easier.
  // "third" = over-the-shoulder (closer + higher than the old 22v rig).
  const viewModeRef = useRef<"cockpit" | "third">("cockpit");
  const [viewMode, setViewMode] = useState<"cockpit" | "third">("cockpit");
  ...
  const buildWorld = useCallback(async (source: File | { layers: Record<string, Int32Array>; meta: Record<string, unknown>; blockCount: number; resolution: number }) => {
    ...
    // anime.js (bundled w/ the three adapter) drives scripted/time-bounded BotW
    // moments (opening cinematic, etc). Continuous sim stays on the tick loop
    // below; anime owns only timing/easing for short sequences, and we freeze
    // the sim (simFrozen) for the duration so no tick writes a property anime
    // is tweening in the same frame. Non-fatal: cinematics fall back to manual
    // easing if the bundle fails to load.
    let animeAnimate: any = null;
    try {
      const animeMod = await import(ANIME_URL);
      animeAnimate = (animeMod as any).animate;
    } catch (e) {
      console.warn("[tinyworld] anime bundle load failed — cinematics fall back to manual easing", e);
    }
    ...
    // ghostMesh, which is the placement-preview outline at the target.
    const carryBlockMat = new THREE.MeshLambertMaterial({ color: 0xffffff });
    const carryBlockMesh = new THREE.Mesh(box, carryBlockMat);
    carryBlockMesh.visible = false;
    scene.add(carryBlockMesh);
    ...
    droneRig.visible = false;
    droneRig.position.set(0, 0, 0);

    // Mini Normandy SR1 silhouette. Local +z is the nose / travel direction:
    // droneYaw = camera.rotation.y + PI maps local +z onto yawForward, so a
    // +z-pointing fuselage flies nose-first. Lit (MeshStandardMaterial) so the
    // hull catches the scene lights and reads as metal, like the reference ship.
    const droneHullMat = new THREE.MeshStandardMaterial({ color: PAL.wall, roughness: 0.88, metalness: 0.02 });
    const droneDarkMat = new THREE.MeshStandardMaterial({ color: 0x6f6559, roughness: 0.9, metalness: 0.02 });
    const droneThrusterMat = new THREE.MeshStandardMaterial({ color: 0x67e8f9, emissive: 0x67e8f9, emissiveIntensity: 1.8, roughness: 0.3, metalness: 0 });

    // Pointed fuselage nose toward +z (cone default apex +y -> rotate to +z).
    const droneFuse = new THREE.Mesh(new THREE.ConeGeometry(voxel * 0.16, voxel * 1.0, 14), droneHullMat);
    droneFuse.rotation.x = -Math.PI / 2;
    droneFuse.position.z = voxel * 0.18;
    // Mid hull block.
    const droneBody = new THREE.Mesh(new THREE.BoxGeometry(voxel * 0.26, voxel * 0.16, voxel * 0.5), droneHullMat);
    droneBody.position.z = -voxel * 0.18;
    // Swept delta wings.
    const droneWingGeo = new THREE.BoxGeometry(voxel * 0.5, voxel * 0.035, voxel * 0.16);
    const droneWingL = new THREE.Mesh(droneWingGeo, droneDarkMat);
    const droneWingR = new THREE.Mesh(droneWingGeo, droneDarkMat);
    droneWingL.position.set(-voxel * 0.34, 0, -voxel * 0.16);
    droneWingR.position.set(voxel * 0.34, 0, -voxel * 0.16);
    droneWingL.rotation.y = 0.45;
    droneWingR.rotation.y = -0.45;
    // Twin engine nacelles at the rear.
    const droneEngGeo = new THREE.CylinderGeometry(voxel * 0.07, voxel * 0.07, voxel * 0.34, 10);
    droneEngGeo.rotateX(Math.PI / 2);
    const droneEngL = new THREE.Mesh(droneEngGeo, droneDarkMat);
    const droneEngR = new THREE.Mesh(droneEngGeo, droneDarkMat);
    droneEngL.position.set(-voxel * 0.16, 0, -voxel * 0.42);
    droneEngR.position.set(voxel * 0.16, 0, -voxel * 0.42);
    // Thruster glow caps facing -z (rear exhaust).
    const droneThrusterGeo = new THREE.CircleGeometry(voxel * 0.07, 12);
    const droneThrusterL = new THREE.Mesh(droneThrusterGeo, droneThrusterMat);
    const droneThrusterR = new THREE.Mesh(droneThrusterGeo, droneThrusterMat);
    droneThrusterL.position.set(-voxel * 0.16, 0, -voxel * 0.59);
    droneThrusterR.position.set(voxel * 0.16, 0, -voxel * 0.59);
    droneThrusterL.rotation.y = Math.PI;
    droneThrusterR.rotation.y = Math.PI;

    // Reused refs for tickDroneRig (pulse + spin targets).
    const droneCore = droneFuse;
    const droneHalo = new THREE.Mesh(
      new THREE.RingGeometry(voxel * 0.5, voxel * 0.62, 20),
      new THREE.MeshBasicMaterial({ color: 0x67e8f9, transparent: true, opacity: 0.4, depthWrite: false, side: THREE.DoubleSide })
    );
    droneHalo.position.z = -voxel * 0.6;
    droneHalo.rotation.x = Math.PI / 2;

    const droneLight = new THREE.PointLight(0x67e8f9, 0.9, voxel * 22, 2);
    droneRig.add(droneBody, droneFuse, droneWingL, droneWingR, droneEngL, droneEngR, droneThrusterL, droneThrusterR, droneHalo, droneLight);
    scene.add(droneRig);
    let droneRigVisible = false;
    const thirdPersonRig = {
      // Over-the-shoulder distance: voxel*22 puts the full ~13.5v sentinel
      // body in frame with margin (was voxel*16, which clipped the camera
      // into the upper torso). The sentinelBodyBlocked guard uses this same
      // value, so wall-collision still tracks the avatar position.
      sentinelForward: voxel * 22,
      droneForward: voxel * 4.5,
      droneDrop: voxel * 0.6,
    };
    const yawForward = () => new THREE.Vector3(-Math.sin(camera.rotation.y), 0, -Math.cos(camera.rotation.y));
    const setDroneRigVisible = (visible: boolean) => {
      if (droneRigVisible === visible) return;
      droneRigVisible = visible;
      droneRig.visible = visible;
      toolGroup.visible = !visible;
    };
    const tickDroneRig = (now: number, dt: number, inLen: number, vert: number) => {
      const active = walkingRef.current && sentinelModeRef.current === "drone";
      setDroneRigVisible(active);
      if (!active) return;
      const forward = yawForward();
      const anchor = camera.position.clone().addScaledVector(forward, thirdPersonRig.droneForward);
      anchor.y -= thirdPersonRig.droneDrop;
      droneRig.position.lerp(anchor, Math.min(1, dt * 16));
      droneYaw += ((camera.rotation.y + Math.PI) - droneYaw) * Math.min(1, dt * 10);
      const pulse = 1 + Math.sin(now * 0.01) * 0.06;
      const bank = Math.max(-0.35, Math.min(0.35, -lookJoystickRef.current.x * 0.18));
      droneBank += (bank - droneBank) * Math.min(1, dt * 9);
      dronePitch += ((vert * -0.12) - dronePitch) * Math.min(1, dt * 8);
      droneCore.scale.setScalar(pulse);
      droneHalo.rotation.z += dt * (2.2 + inLen * 2.6 + Math.abs(vert) * 1.2);
      // Single rotation write per frame. YXZ order applies yaw (world up)
      // first, then pitch, then bank in the yawed frame — aircraft/Luanti
      // content_cao convention, no quaternion-vs-Euler double-apply.
      droneRig.rotation.order = "YXZ";
      droneRig.rotation.set(dronePitch, droneYaw, droneBank);
      const thrustGlow = Math.min(1, inLen + Math.abs(vert));
      droneCore.material.opacity = 0.72 + thrustGlow * 0.23;
      droneHalo.material.opacity = 0.35 + thrustGlow * 0.35;
      droneLight.intensity = 0.35 + thrustGlow * 0.75;
    };
    ...
    type SettleFx = { mesh: any; slot: number; vx: number; vy: number; vz: number; layer: string; age: number; shadow: any };
    type DustFx = { mesh: any; age: number };
    type RelocationTetherFx = { line: any; age: number };
    const settleFx: SettleFx[] = [];
    const dustFx: DustFx[] = [];
    const relocationTethers: RelocationTetherFx[] = [];
    const dustGeo = new THREE.CircleGeometry(voxel * 0.55, 18);
    const dustPool: any[] = [];
    const shadowPool: any[] = [];
    const tetherPool: any[] = [];
    for (let i = 0; i < 24; i++) {
      const dust = new THREE.Mesh(
        dustGeo,
        new THREE.MeshBasicMaterial({ color: 0xd8c6a0, transparent: true, opacity: 0, depthWrite: false })
      );
      ...
      const sh = new THREE.Mesh(
        new THREE.CircleGeometry(voxel * 0.55, 20),
    ...
    for (let i = 0; i < 12; i++) {
      ...
      const line = new THREE.Line(g, new THREE.LineBasicMaterial({ color: 0xffd75e, transparent: true, opacity: 0, depthTest: false }));
      line.visible = false;
      line.renderOrder = 20;
      scene.add(line);
    ...
    const spawnDustFx = (vx: number, vy: number, vz: number) => {
      ...
      if (!dust) return;
      dust.position.set((vx - cxRound) * voxel, (vy - 0.49) * voxel, (vz - czRound) * voxel);
      dust.scale.setScalar(0.45);
      dust.material.opacity = 0.34;
      dust.visible = true;
      dustFx.push({ mesh: dust, age: 0 });
    };
    const spawnRelocationTether = (src: { x: number; y: number; z: number } | null | undefined, tx: number, ty: number, tz: number) => {
      if (!src) return;
      const line = takeFx(tetherPool);
      if (!line) return;
      const arr = line.geometry.attributes.position.array as Float32Array;
      arr[0] = (src.x - cxRound) * voxel; arr[1] = src.y * voxel; arr[2] = (src.z - czRound) * voxel;
      arr[3] = (tx - cxRound) * voxel; arr[4] = ty * voxel; arr[5] = (tz - czRound) * voxel;
      line.geometry.attributes.position.needsUpdate = true;
      line.material.opacity = 0.75;
      line.visible = true;
      relocationTethers.push({ line, age: 0 });
    };
    const spawnSettleFx = (mesh: any, slot: number, vx: number, vy: number, vz: number, layer: string, source?: { x: number; y: number; z: number } | null) => {
      setBlockMatrix(mesh, slot, vx, vy, vz, 0.82, voxel * 0.75);
      const shadow = takeFx(shadowPool);
      if (!shadow) return;
      shadow.position.set((vx - cxRound) * voxel, (vy - 0.5) * voxel + 0.003, (vz - czRound) * voxel);
      shadow.scale.setScalar(0.4);
      shadow.material.opacity = 0.22;
      shadow.visible = true;
      settleFx.push({ mesh, slot, vx, vy, vz, layer, age: 0, shadow });
      spawnDustFx(vx, vy, vz);
      spawnRelocationTether(source, vx, vy, vz);
    };
    let _fxPrev = 0;
    const tickPhysicalFx = (now: number) => {
      const dt = _fxPrev ? Math.min((now - _fxPrev) / 1000, 0.05) : 0.016;
      _fxPrev = now;
      for (let i = settleFx.length - 1; i >= 0; i--) {
        const fx = settleFx[i];
        const t = Math.min(1, fx.age + dt / 0.43);
        fx.age = t;
        const ease = 1 - Math.pow(1 - t, 3);
        const bounce = Math.sin(t * Math.PI) * 0.08;
        setBlockMatrix(fx.mesh, fx.slot, fx.vx, fx.vy, fx.vz, 0.82 + 0.18 * ease + bounce, voxel * 0.75 * (1 - ease));
        fx.shadow.scale.setScalar(0.4 + 0.65 * ease);
        fx.shadow.material.opacity = 0.22 * (1 - Math.max(0, t - 0.55) / 0.45);
        if (t >= 1) {
          setBlockMatrix(fx.mesh, fx.slot, fx.vx, fx.vy, fx.vz, 1, 0);
          fx.shadow.visible = false;
          settleFx.splice(i, 1);
        }
      }
      for (let i = dustFx.length - 1; i >= 0; i--) {
        const fx = dustFx[i];
        const t = Math.min(1, fx.age + dt / 0.52);
        fx.age = t;
        fx.mesh.scale.setScalar(0.45 + t * 1.25);
        fx.mesh.material.opacity = 0.34 * (1 - t);
        if (t >= 1) { fx.mesh.visible = false; dustFx.splice(i, 1); }
      }
      for (let i = relocationTethers.length - 1; i >= 0; i--) {
        const fx = relocationTethers[i];
        const t = Math.min(1, fx.age + dt / 0.36);
        fx.age = t;
        fx.line.material.opacity = 0.75 * (1 - t);
        if (t >= 1) { fx.line.visible = false; relocationTethers.splice(i, 1); }
      }
    };

    // Apply per-instance tone jitter to a single slot on dynamic place/move.
    // Mirrors addLayer's _toneFor so freshly-placed blocks match the baked
    // terrain look instead of the flat material base color. Reads the layer
    // base tone from the mesh's own material.color (set by addLayer == _baseTone).
    const _sstTone = new THREE.Color();
    const _sstBase = new THREE.Color();
    const setSlotTone = (mesh: any, slot: number, vx: number, vy: number, vz: number, layer: string) => {
      const layerName = layer || mesh.userData?.layer || "block";
      const matColor = mesh.material && mesh.material.color;
      _sstBase.copy(matColor || _sstBase.setHex(0xffffff));
      if (layerName === "water") {
        mesh.setColorAt(slot, _sstBase);
      } else {
        const h = (Math.imul(vx, 73856093) ^ Math.imul(vy, 19349663) ^ Math.imul(vz, 83492791)) >>> 0;
        const n = (h % 1000) / 1000;
        const lift = layerName === "snow" ? 0.04 : layerName === "leaves" ? 0.08 : 0.06;
        const v = 0.94 + n * lift + ((h >>> 10) % 3) * 0.012;
        _sstTone.copy(_sstBase).multiplyScalar(v);
        if ((h & 7) === 0 && layerName !== "wall" && layerName !== "ceiling") _sstTone.offsetHSL(0, 0.02, 0.018);
        mesh.setColorAt(slot, _sstTone);
      }
    ...
    const addLayer = (buffer: ArrayBuffer | Int32Array | undefined, color: number, opacity = 1, layerName = "block", hiddenBuf?: ArrayBuffer | Int32Array) => {
      ...
      const _baseTone = new THREE.Color(color);
      const _tone = new THREE.Color(color);
      const _toneFor = (x: number, y: number, z: number) => {
        if (layerName === "water") return _baseTone;
        const h = (Math.imul(x, 73856093) ^ Math.imul(y, 19349663) ^ Math.imul(z, 83492791)) >>> 0;
        const n = (h % 1000) / 1000;
        const lift = layerName === "snow" ? 0.04 : layerName === "leaves" ? 0.08 : 0.06;
        const v = 0.94 + n * lift + ((h >>> 10) % 3) * 0.012;
        _tone.copy(_baseTone).multiplyScalar(v);
        if ((h & 7) === 0 && layerName !== "wall" && layerName !== "ceiling") _tone.offsetHSL(0, 0.02, 0.018);
        return _tone;
      };
      for (let i = 0, j = 0; i < arr.length; i += 3, j++) {
        ...
        mesh.setColorAt(j, _toneFor(x, y, z));
    ...
    // âââ Offline Catch-up Simulation âââââââââââââââââââââââââââââââââââââââââââ
    ...
    if (catchUpSummary?.rift) {
      ...
      try { orbAlertRef.current?.("rift", { x, y, z }); } catch (e) {}
    }
    ...
    // View-aware camera lift: third-person rides higher (14.5v eye) so you look
    // DOWN onto the mech; cockpit stays at head level (11.5v). All 4 camera
    // foot-reference sites use this so grounding is preserved when the eye
    // rises (foot = camera.y - EYE_HEIGHT - camLift stays world-fixed).
    const camLiftFor = () => viewModeRef.current === "third" ? voxel * 11 : WALK_CAMERA_LIFT;
    ...
    // 3D collision probe for the drone. The drone flies freely, so it must not
    // phase through walls/ceilings/floors. Tests a small horizontal square
    // around the probe point against the column at the drone's voxel Y (and
    // the voxel above, to catch thin ceilings) — cheap and good enough to stop
    // the drone from clipping into geometry.
    const droneR = voxel * 0.45;
    const droneBlockedAt = (wx: number, wy: number, wz: number): boolean => {
      const vy = Math.round(wy / voxel);
      const minVX = Math.round((wx - droneR) / voxel) + cxRound;
      const maxVX = Math.round((wx + droneR) / voxel) + cxRound;
      const minVZ = Math.round((wz - droneR) / voxel) + czRound;
      const maxVZ = Math.round((wz + droneR) / voxel) + czRound;
      for (let vx = minVX; vx <= maxVX; vx++) {
        for (let vz = minVZ; vz <= maxVZ; vz++) {
          const col = colMap.get(vx + "," + vz);
          if (!col) continue;
          if (col.has(vy) || col.has(vy + 1)) return true;
        }
      }
      return false;
    };

    // Sentinel body-ahead collision guard. The sentinel avatar is rendered
    // ~sentinelForward voxels ahead of the camera, so the camera-body test
    // alone lets the visible figure clip into walls. This probes the voxel the
    // body stands in (and the two above) and reports whether the figure would
    // be embedded in solid geometry.
    const sentinelBodyBlocked = (bx: number, bz: number, footWy: number): boolean => {
      const bVX = Math.round(bx / voxel) + cxRound;
      const bVZ = Math.round(bz / voxel) + czRound;
      const bVY = Math.round(footWy / voxel);
      const col = colMap.get(bVX + "," + bVZ);
      if (!col) return false;
      return col.has(bVY) || col.has(bVY + 1) || col.has(bVY + 2);
    };
    ...
    // ── BotW block feel: hover-lift on grab + snap-to-place on release ──
    // anime.js owns the eased progress (carryAnim.k 0->1); the per-frame carry
    // block reads k and positions carryBlockMesh along the arc. No simFrozen is
    // needed: no continuous-sim tick writes carryBlockMesh, so there is no
    // same-frame double-write to guard against (unlike the opening cinematic,
    // which moves the camera that walk-physics owns every frame).
    const LIFT_MS = 280;
    const SNAP_MS = 190;
    type CarryAnim = { phase: "lift" | "snap"; from: THREE.Vector3; to: THREE.Vector3; k: number; onDone?: () => void };
    let carryAnim: CarryAnim | null = null;
    const carryHoldPos = () => {
      ...
      return camera.position.clone()
        .addScaledVector(forward, voxel * 1.7)
        .addScaledVector(right, voxel * 0.55)
        .add(new THREE.Vector3(0, -voxel * 0.62, 0));
    };
    const startCarryAnim = (phase: "lift" | "snap", from: THREE.Vector3, to: THREE.Vector3, ms: number, onDone?: () => void) => {
      carryAnim = { phase, from: from.clone(), to: to.clone(), k: 0, onDone };
      if (animeAnimate) {
        try { animeAnimate(carryAnim, { k: 1, duration: ms, ease: phase === "lift" ? "out(2)" : "inOut(2)" }); }
        catch (e) { carryAnim.k = 1; }
      } else {
        carryAnim.k = 1;
      }
    };
    const tickCarryAnim = () => {
      ...
      const arc = a.phase === "lift" ? Math.sin(Math.PI * k) * voxel * 0.6 : 0;
      carryBlockMesh.position.set(f.x + (to.x - f.x) * k, f.y + (to.y - f.y) * k + arc, f.z + (to.z - f.z) * k);
      if (k >= 1) {
        carryAnim = null;
        a.onDone?.();
      }
    ...
    const completePickup = () => {
      ...
      ghostMesh.visible = false;
      setCarryLayer(layer);
      // Hover-lift (Part 1): the grabbed block floats from its origin to a
      // hold position in front of the camera. LARGE-mode bulk harvest below
      // feeds stockpile invisibly; only this one carried block lifts.
      {
        const originWorld = new THREE.Vector3((vx - cxRound) * voxel, vy * voxel, (vz - czRound) * voxel);
        (carryBlockMesh.material as any).color.set(color);
        carryBlockMesh.position.copy(originWorld);
        carryBlockMesh.rotation.set(0, 0, 0);
        carryBlockMesh.visible = true;
        startCarryAnim("lift", originWorld, carryHoldPos(), LIFT_MS);
      }
    ...
    const bulkPlaceAround = (cx: number, cy: number, cz: number, preferLayer: string, skipSelf: boolean, source?: { x: number; y: number; z: number } | null): number => {
      let placed = 0;
      for (let dx = -1; dx <= 2; dx++) {
        for (let dy = -1; dy <= 2; dy++) {
          for (let dz = -1; dz <= 2; dz++) {
    ...
    const placeAt = (vx: number, vy: number, vz: number) => {
      ...
      setSlotTone(carryState.mesh, slot, vx, vy, vz, carryState.layer);
      ...
      spawnSettleFx(carryState.mesh, slot, vx, vy, vz, carryState.layer, { x: carryState.originVX, y: carryState.originVY, z: carryState.originVZ });
    ...
    const tryPlace = () => {
      ...
      if (carryAnim) return; // mid lift/snap — ignore until it settles
      const target = getPlacementTarget();
      if (!target) return;
      const tVx = target.vx, tVy = target.vy, tVz = target.vz;
      const tWorld = target.world.clone();
      const from = carryBlockMesh.position.clone();
      // Snap-to-place (Part 2): the held block flies from hold to the grid-
      // snapped target, then commits to the instanced mesh + ledger + FX.
      startCarryAnim("snap", from, tWorld, SNAP_MS, () => {
        if (!carryState) return; // cancelled mid-snap
        const placedLayer = carryState.layer;
        const placedSource = { x: carryState.originVX, y: carryState.originVY, z: carryState.originVZ };
        if (placeAt(tVx, tVy, tVz)) {
          carryBlockMesh.visible = false;
          carryState = null;
          ghostMesh.visible = false;
          setCarryLayer(null);
          ...
          if (sentinelModeRef.current === "large") {
            bulkPlaceAround(tVx, tVy, tVz, placedLayer, true, placedSource);
          }
          settleWaterNear(tVx, tVy, tVz);
        }
      });
    };
    ...
    const returnCarry = () => {
      if (!carryState) return;
      carryAnim = null; // cancel any in-flight lift/snap
    ...
    const onKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.code === "Escape" && pilotActiveRef.current) { exitPilotRef.current?.(true); return; }
    ...
    const spawnVoidCreature = (vx: number, vy: number, vz: number) => {
      ...
      try { orbAlertRef.current?.("void_creature"); } catch (e) {}
      return c;
    };
    ...
    const scanNewLand = () => {
      ...
      try { orbAlertRef.current?.("scan_complete"); } catch (e) {}
      scanCount += 1;
    ...
    const detonate = (bomb: Bomb, visited: Set<Bomb>): any => {
      ...
      detonationsTotal++;
      try { orbAlertRef.current?.("bomb_detonated"); } catch (e) {}
      setLedger({ ...L });
      updateBombUi();
    ...
    // ── ORB COMPANION + OPENING + TUTORIAL + PILOT (BotW-inspired) ─────────────
    // A floating guide orb (Navi analog) that hovers at the player's shoulder,
    // speaks typewriter dialogue, tints by mood, and fires alert lines on world
    // events. Drives the first-load cinematic and a contained void-attack
    // tutorial. Also hosts the player-piloted ship loop (chase cam + physics).
    // Live API: __tw.orb.{say,alert,mood,show,flush,play,tutor},
    //           __tw.pilot.{enter,exit,active}, __tw.opening.play
    type OrbMood = "calm" | "alert" | "urgent" | "joy" | "dim";
    const ORB_MOOD_COLOR: Record<OrbMood, number> = {
      calm: 0xffd75e, alert: 0xffb020, urgent: 0xff3fa4, joy: 0x6ee7a8, dim: 0x6a7079,
    };
    const orbRef = {
      visible: false,
      mood: "calm" as OrbMood,
      ...
      light: null as any,
      halo: null as any,
      lineQueue: [] as Array<{ text: string; mood: OrbMood; holdMs: number }>,
      cur: null as null | { text: string; mood: OrbMood; holdMs: number; t0: number; chars: number; done: boolean; doneAt: number },
      lastUiText: "",
      prev: 0,
    };
    ...
    const makeOrbBody = () => {
      const group = new THREE.Group();
      const core = new THREE.Mesh(new THREE.IcosahedronGeometry(voxel * 0.55, 1), new THREE.MeshBasicMaterial({ color: ORB_MOOD_COLOR.calm }));
      const glow = new THREE.Mesh(new THREE.SphereGeometry(voxel * 1.15, 16, 16), new THREE.MeshBasicMaterial({ color: ORB_MOOD_COLOR.calm, transparent: true, opacity: 0.2, depthWrite: false }));
      const halo = new THREE.Mesh(new THREE.RingGeometry(voxel * 0.9, voxel * 1.18, 32), new THREE.MeshBasicMaterial({ color: ORB_MOOD_COLOR.calm, transparent: true, opacity: 0.5, depthWrite: false, side: THREE.DoubleSide }));
      const light = new THREE.PointLight(ORB_MOOD_COLOR.calm, 1.3, voxel * 32, 1.6);
      group.add(glow); group.add(core); group.add(halo); group.add(light);
      group.userData = { glow };
      scene.add(group);
      orbRef.group = group; orbRef.core = core; orbRef.light = light; orbRef.halo = halo;
      return group;
    };
    const orbSetMood = (m: OrbMood) => {
      orbRef.mood = m;
      const c = ORB_MOOD_COLOR[m];
      if (orbRef.core) orbRef.core.material.color.setHex(c);
      if (orbRef.group?.userData?.glow) orbRef.group.userData.glow.material.color.setHex(c);
      if (orbRef.halo) orbRef.halo.material.color.setHex(c);
      if (orbRef.light) orbRef.light.color.setHex(c);
    };
    const orbShow = (v: boolean) => {
      orbRef.visible = v;
      if (orbRef.group) orbRef.group.visible = v;
    };
    const orbSay = (text: string, mood: OrbMood = "calm", holdMs = 4200) => {
      orbRef.lineQueue.push({ text, mood, holdMs });
    };
    const orbFlush = () => {
      orbRef.lineQueue.length = 0;
      orbRef.cur = null;
      orbRef.lastUiText = "";
      setOrbUi({ text: "", mood: orbRef.mood, visible: false });
    };
    const ORB_ALERT_LINES: Record<string, Array<{ text: string; mood: OrbMood }>> = {
      rift: [
        { text: "A rift opened. The void is hungry tonight.", mood: "urgent" },
        { text: "There \u2014 a breach. The world is bleeding blocks.", mood: "urgent" },
        { text: "I feel a tear in the lattice. Stay sharp.", mood: "alert" },
      ],
      void_creature: [
        { text: "A wraith. It's eating the edges.", mood: "alert" },
        { text: "Something drifts in from the dark. Don't let it settle.", mood: "alert" },
      ],
      bomb_detonated: [
        { text: "The Return. Strain back to soil.", mood: "joy" },
        { text: "Beautiful. Everything disordered, nothing lost.", mood: "joy" },
      ],
      ship_stranded: [
        { text: "The ship's out of hull to burn. It's stranded.", mood: "urgent" },
        { text: "No mass left to fly. She won't move again.", mood: "urgent" },
      ],
      ship_low_fuel: [
        { text: "Hull's running thin. Mind the fuel.", mood: "alert" },
        { text: "She's burning her own keel. Find a place to land.", mood: "alert" },
      ],
      ship_arrived: [
        { text: "We're here.", mood: "calm" },
        { text: "Landed. Solid ground again.", mood: "joy" },
      ],
      scan_complete: [
        { text: "New ground mapped. The world grew.", mood: "joy" },
        { text: "Scanned. More matter for the colony.", mood: "calm" },
      ],
      tutorial_start: [{ text: "First lesson. The void sends a scout.", mood: "alert" }],
      tutorial_win_bomb: [{ text: "You Returned it. That's how we hold the line.", mood: "joy" }],
      tutorial_win_scout: [{ text: "You saw it off. Good eye.", mood: "joy" }],
      tutorial_win_wait: [{ text: "It drifted off on its own. Lucky. Next time, act.", mood: "calm" }],
      opening_done: [{ text: "The world is yours now. I'll watch with you.", mood: "calm" }],
    };
    const orbAlert = (kind: string, _ctx?: any) => {
      const lines = ORB_ALERT_LINES[kind];
    ...
    const tickOrb = (now: number) => {
      ...
      const dt = Math.min((now - orbRef.prev) / 1000, 0.05);
      orbRef.prev = now;

      const isDrone = sentinelModeRef.current === "drone";
      const activeCenter = isDrone ? droneRig.position : sentinelGroup.position;
      const activeUp = isDrone ? voxel * 1.15 : voxel * 1.85;
      const activeForward = new THREE.Vector3();
      const activeRight = new THREE.Vector3();
      if (isDrone) {
        activeForward.set(-Math.sin(droneYaw), 0, -Math.cos(droneYaw));
      } else {
      ...
      orbRef.group.position.set(orbRef.pos.x, orbRef.pos.y + bob, orbRef.pos.z);
      orbRef.core.scale.setScalar(pulse);
      orbRef.halo.lookAt(camera.position);
      // Typewriter advance.
      if (!orbRef.cur && orbRef.lineQueue.length > 0) {
        const next = orbRef.lineQueue.shift() as { text: string; mood: OrbMood; holdMs: number };
        orbRef.cur = { text: next.text, mood: next.mood, holdMs: next.holdMs, t0: now, chars: 0, done: false, doneAt: 0 };
        orbSetMood(next.mood);
      }
      if (orbRef.cur) {
        const c = orbRef.cur;
        if (!c.done) {
          const want = Math.floor((now - c.t0) / 26); // ~38 cps
          if (want !== c.chars) {
            c.chars = Math.min(want, c.text.length);
            if (c.chars >= c.text.length) { c.done = true; c.doneAt = now; }
            const shown = c.text.slice(0, c.chars);
            if (shown !== orbRef.lastUiText) {
              orbRef.lastUiText = shown;
              setOrbUi({ text: shown, mood: c.mood, visible: true });
            }
          }
        } else if (now - c.doneAt > c.holdMs) {
          orbRef.cur = null;
          orbRef.lastUiText = "";
          setOrbUi({ text: "", mood: orbRef.mood, visible: false });
        }
    ...
    // ── First-load opening cinematic ────────────────────────────────────────
    const openingRef = { active: false, t0: 0, step: 0, done: false };
    // simFrozen: while true the main loop skips continuous sim ticks + walk
    // physics so an anime.js cinematic can own the camera/world without any
    // tick fighting it for the same property in the same frame.
    let simFrozen = false;
    // cineK: normalized 0->1 progress anime.js tweens for the opening camera
    // move. tickOpening reads it each frame and maps it onto the arc path.
    const cineK = { k: 0 };
    const startOpening = () => {
      if (openingRef.active || openingRef.done) return;
      try { if (localStorage.getItem("tw_opening_seen")) { openingRef.done = true; return; } } catch (e) {}
      openingRef.active = true; openingRef.t0 = performance.now(); openingRef.step = 0;
      orbit.enabled = false;
      if (fp.isLocked) { try { fp.unlock(); } catch (e) {} }
      setOpeningUi({ active: true, step: 0 });
      orbSay("You're here. I wasn't sure you'd come.", "calm", 3600);
      // Hand the camera move's timing/easing to anime.js and freeze sim for
      // the cinematic. tickOpening maps cineK onto the arc; if anime didn't
      // load, tickOpening falls back to manual easing off `t`.
      simFrozen = true;
      cineK.k = 0;
      if (animeAnimate) {
        try { animeAnimate(cineK, { k: 1, duration: 5500, ease: "out(3)" }); } catch (e) { cineK.k = 1; }
      } else {
        cineK.k = 1;
      }
    };
    const endOpening = () => {
      openingRef.active = false; openingRef.done = true;
      simFrozen = false;
      setOpeningUi({ active: false, step: 0 });
      try { localStorage.setItem("tw_opening_seen", "1"); } catch (e) {}
      orbit.enabled = true;
      orbAlert("opening_done");
      setTimeout(() => startTutorial(), 2600);
    };
    const tickOpening = (now: number) => {
      if (!openingRef.active) return;
      const t = (now - openingRef.t0) / 1000;
      // Hero camera: high overhead easing down to a 3/4 view of the world.
      const cx = (0 - cxRound) * voxel;
      const cz = (0 - czRound) * voxel;
      const cyTop = span * voxel * 1.6;
      const cyHero = span * voxel * 0.9;
      // anime.js owns the easing via cineK (tweened in startOpening); fall
      // back to the old manual ease off elapsed time if the bundle didn't load.
      const ease = animeAnimate
        ? Math.max(0, Math.min(1, cineK.k))
        : 1 - Math.pow(1 - Math.min(1, t / 5.5), 3);
      const angle = -Math.PI * 0.25 + ease * Math.PI * 0.6;
      const rad = span * voxel * (1.5 - 0.5 * ease);
      const y = cyTop + (cyHero - cyTop) * ease;
      camera.position.set(cx + Math.cos(angle) * rad, y, cz + Math.sin(angle) * rad);
      camera.lookAt(cx, span * voxel * 0.2, cz);
      const beats: Array<{ at: number; fn: () => void }> = [
        { at: 1.6, fn: () => orbSay("This is what's left. A scanned sliver of a real place.", "calm", 4000) },
        { at: 6.0, fn: () => orbSay("The Void eats at its edges when we're not looking.", "alert", 4000) },
        { at: 10.5, fn: () => orbSay("That ship is yours. And so is the sky.", "joy", 3800) },
        { at: 14.5, fn: () => orbSay("Shall we? I'll teach you to hold the line.", "calm", 3600) },
      ];
      for (let i = 0; i < beats.length; i++) {
        if (openingRef.step === i && t >= beats[i].at) { beats[i].fn(); openingRef.step = i + 1; }
      }
      if (t >= 18.5) endOpening();
    };

    // ── Contained void-attack tutorial ──────────────────────────────────────
    const tutorialRef = { active: false, t0: 0, creature: null as any, won: false, mode: "" as "" | "bomb" | "scout" | "wait" };
    const startTutorial = () => {
      if (tutorialRef.active) return;
      const wf = weakestFrontier(6, performance.now());
      const picks = wf ? wf.picks : null;
      if (!picks || picks.length === 0) { orbSay("No edge to defend yet \u2014 scan more ground first.", "dim"); return; }
      const cx = (0 - cxRound) * voxel, cz = (0 - czRound) * voxel;
      let best = picks[0], bestD = Infinity;
      for (const p of picks) {
        const d = Math.hypot((p.vx - cxRound) * voxel - cx, (p.vz - czRound) * voxel - cz);
        if (d < bestD) { bestD = d; best = p; }
      }
      const c = spawnVoidCreature(best.vx, best.vy + 2, best.vz);
      tutorialRef.creature = c; tutorialRef.active = true; tutorialRef.t0 = performance.now(); tutorialRef.won = false; tutorialRef.mode = "";
      orbAlert("tutorial_start");
      orbSay("Scout it from the air, or bomb it back to soil. Your call.", "alert", 5200);
    };
    const tickTutorial = (now: number) => {
      if (!tutorialRef.active) return;
      const stillThere = tutorialRef.creature && voidCreatures.indexOf(tutorialRef.creature) >= 0;
      if (tutorialRef.creature && !stillThere && !tutorialRef.won) {
        tutorialRef.won = true;
        const mode = detonationsTotal > 0 ? "bomb" : (ships.some((s: any) => s.flying) ? "scout" : "wait");
        tutorialRef.mode = mode as any;
        orbAlert(mode === "bomb" ? "tutorial_win_bomb" : mode === "scout" ? "tutorial_win_scout" : "tutorial_win_wait");
        setTimeout(() => { tutorialRef.active = false; }, 5000);
      }
      if (!tutorialRef.won && now - tutorialRef.t0 > 60000) {
        if (tutorialRef.creature && voidCreatures.indexOf(tutorialRef.creature) >= 0) despawnVoidCreature(tutorialRef.creature);
        tutorialRef.won = true; tutorialRef.mode = "wait";
        orbAlert("tutorial_win_wait");
        setTimeout(() => { tutorialRef.active = false; }, 4000);
      }
    };

    // ── Player-piloted ship (BotW-style chase cam + floaty physics) ─────────
    const pilotRef = {
      active: false, idx: 0, t0: 0,
      vel: new THREE.Vector3(),
      yaw: 0, pitch: 0, roll: 0,
      camSaved: { pos: new THREE.Vector3(), quat: new THREE.Quaternion() } as any,
      mouseYaw: 0, mousePitch: 0,
      lastFuelAlert: 0,
    };
    let pilotMouseHandler: ((e: MouseEvent) => void) | null = null;
    const enterPilot = (idx = 0) => {
      if (!ships[idx]) {
        const built = buildShip();
        if (!built || !built.ok) { orbSay("I can't make a ship yet \u2014 " + (built && (built as any).reason || "no hull"), "urgent"); return; }
        idx = ships.length - 1;
      }
      const ship = ships[idx];
      pilotRef.active = true; pilotRef.idx = idx; pilotRef.t0 = performance.now();
      pilotActiveRef.current = true;
      pilotRef.vel.set(0, 0, 0); pilotRef.roll = 0;
      pilotRef.mouseYaw = 0; pilotRef.mousePitch = 0;
      const shipWX = (ship.x - cxRound) * voxel, shipWZ = (ship.z - czRound) * voxel;
      pilotRef.yaw = Math.atan2(shipWZ, -shipWX); // prow faces world center
      pilotRef.pitch = -0.15;
      pilotRef.camSaved.pos.copy(camera.position);
      camera.getWorldQuaternion(pilotRef.camSaved.quat);
      orbit.enabled = false;
      if (fp.isLocked) { try { fp.unlock(); } catch (e) {} }
      pilotMouseHandler = (e: MouseEvent) => {
        if (!pilotRef.active) return;
        pilotRef.mouseYaw -= e.movementX * 0.0022;
        pilotRef.mousePitch -= e.movementY * 0.0022;
        pilotRef.mousePitch = Math.max(-1.2, Math.min(1.2, pilotRef.mousePitch));
      };
      renderer.domElement.addEventListener("mousemove", pilotMouseHandler);
      try { (renderer.domElement as any).requestPointerLock?.(); } catch (e) {}
      ship.flying = true;
      setPilotUi({ active: true, fuel: ship.fuel, maxFuel: SHIP_HULL_STONE, speed: 0, altitude: ship.group.position.y / voxel, banking: 0 });
      orbSay("Lift-off. W pushes forward, mouse steers, Space climbs.", "joy", 5200);
    };
    const exitPilot = (land = true) => {
      if (!pilotRef.active) return;
      const ship = ships[pilotRef.idx];
      pilotRef.active = false;
      pilotActiveRef.current = false;
      if (pilotMouseHandler) { renderer.domElement.removeEventListener("mousemove", pilotMouseHandler); pilotMouseHandler = null; }
      try { document.exitPointerLock?.(); } catch (e) {}
      if (ship) {
        ship.flying = false;
        if (land) {
          const vx = Math.round(ship.x), vz = Math.round(ship.z);
          const top = colTop(vx, vz) ?? 0;
          ship.x = vx; ship.z = vz;
          ship.group.position.set((vx - cxRound) * voxel, (top + 4) * voxel, (vz - czRound) * voxel);
        }
      }
      orbit.enabled = true;
      if (ship) {
        const vx = Math.round(ship.x), vz = Math.round(ship.z);
        const top = colTop(vx, vz) ?? 0;
        camera.position.set((vx - cxRound) * voxel, (top + 30) * voxel, (vz - czRound) * voxel + voxel * 18);
        camera.lookAt((vx - cxRound) * voxel, (top + 4) * voxel, (vz - czRound) * voxel);
      }
      orbit.target.set(0, 0, 0);
      setPilotUi({ active: false, fuel: ship ? ship.fuel : 0, maxFuel: SHIP_HULL_STONE, speed: 0, altitude: 0, banking: 0 });
      if (ship) orbAlert("ship_arrived");
    };
    const tickPilot = (now: number, dt: number) => {
      if (!pilotRef.active) return;
      const ship = ships[pilotRef.idx];
      if (!ship) { pilotRef.active = false; return; }
      if (ship.fuel < 1) {
        pilotRef.vel.multiplyScalar(0.9);
        if (now - pilotRef.lastFuelAlert > 4000) { orbAlert("ship_stranded"); pilotRef.lastFuelAlert = now; }
        if (pilotRef.vel.length() < 0.2) { exitPilot(true); return; }
      }
      ...
      pilotRef.yaw += pilotRef.mouseYaw + yawIn * dt * 1.2;
      pilotRef.pitch += pilotRef.mousePitch;
      pilotRef.mouseYaw = 0; pilotRef.mousePitch = 0;
      pilotRef.pitch = Math.max(-1.2, Math.min(1.2, pilotRef.pitch));
      const targetRoll = yawIn * 0.5;
      pilotRef.roll += (targetRoll - pilotRef.roll) * Math.min(1, dt * 4);
      const fwd = new THREE.Vector3(
        Math.cos(pilotRef.yaw) * Math.cos(pilotRef.pitch),
        Math.sin(pilotRef.pitch),
        -Math.sin(pilotRef.yaw) * Math.cos(pilotRef.pitch),
      );
      const ACCEL = voxel * 6.0;
      const MAXSPEED = voxel * 14;
      pilotRef.vel.addScaledVector(fwd, thrust * ACCEL * dt);
      pilotRef.vel.y += vert * ACCEL * 0.8 * dt;
      pilotRef.vel.multiplyScalar(1 - Math.min(1, dt * 0.8));
      if (Math.abs(vert) < 0.01 && Math.abs(thrust) < 0.01) pilotRef.vel.y -= voxel * 1.2 * dt;
      const sp = pilotRef.vel.length();
      if (sp > MAXSPEED) pilotRef.vel.multiplyScalar(MAXSPEED / sp);
      const wx = (ship.x - cxRound) * voxel + pilotRef.vel.x * dt;
      const wz = (ship.z - cxRound) * voxel + pilotRef.vel.z * dt;
      let wy = ship.group.position.y + pilotRef.vel.y * dt;
      const vxN = Math.round(ship.x), vzN = Math.round(ship.z);
      const gtop = (colTop(vxN, vzN) ?? 0) * voxel;
      if (wy < gtop + voxel * 3) { wy = gtop + voxel * 3; if (pilotRef.vel.y < 0) pilotRef.vel.y = 0; }
      ship.x = cxRound + wx / voxel;
      ship.z = czRound + wz / voxel;
      ship.group.position.set(wx, wy, wz);
      ship.group.rotation.set(pilotRef.pitch * 0.5, pilotRef.yaw, pilotRef.roll);
      const moved = pilotRef.vel.length() * dt / voxel;
      ship.cellsSinceBurn += moved;
      if (ship.cellsSinceBurn >= SHIP_CELLS_PER_BURN && ship.fuel > 0) {
        const burns = Math.floor(ship.cellsSinceBurn / SHIP_CELLS_PER_BURN);
        ship.cellsSinceBurn -= burns * SHIP_CELLS_PER_BURN;
        const burn = Math.min(burns, ship.fuel);
        ship.fuel -= burn;
        ledgerMove("built", "void", burn, "ship_fly", "stone");
        if (ship.fuel <= 3 && now - pilotRef.lastFuelAlert > 6000) { orbAlert("ship_low_fuel"); pilotRef.lastFuelAlert = now; }
      }
      const camOff = new THREE.Vector3(-Math.cos(pilotRef.yaw), 0.45, Math.sin(pilotRef.yaw)).multiplyScalar(voxel * 7);
      const camTarget = ship.group.position.clone().add(camOff);
      camera.position.lerp(camTarget, Math.min(1, dt * 6));
      const lookAt = ship.group.position.clone().add(fwd.clone().multiplyScalar(voxel * 6));
      camera.lookAt(lookAt);
      setPilotUi({ active: true, fuel: ship.fuel, maxFuel: SHIP_HULL_STONE, speed: sp / voxel, altitude: wy / voxel, banking: pilotRef.roll });
    };

    // Expose engines to the ref bridge so earlier-defined call sites (rift
    // spawn, void spawn, scan, ship system, onLoadSelected) can fire alerts
    // and trigger the opening without TDZ ordering concerns.
    orbAlertRef.current = orbAlert;
    startOpeningRef.current = startOpening;
    enterPilotRef.current = enterPilot;
    exitPilotRef.current = exitPilot;
    // Escape exits pilot mode (and snap-lands).
    // (handled in the keydown wiring below)
    ...
    const __twDebugEl = (() => {
      ...
      panel.style.cssText = [
        ...
        "color:#9fffe8",
        "border:1px solid rgba(111,231,168,0.35)",
      ...
      title.textContent = "TINYWORLD DEBUG";
      title.style.cssText = "font-weight:800;letter-spacing:2px;color:#fff;margin-bottom:8px";
      const readout = document.createElement("pre");
      readout.style.cssText = "margin:0;white-space:pre-wrap;line-height:1.35;color:#9fffe8";
      const buttons = document.createElement("div");
      buttons.style.cssText = "display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:6px;margin-top:10px";
      const actions: Array<[string, () => unknown]> = [
        ["settle fx", () => (window as any).__tw?.settleTest?.()],
        ["toggle drone", () => {
          const mode = (window as any).__tw?.droneInfo?.()?.mode === "drone" ? "large" : "drone";
          return (window as any).__tw?.setSentinelMode?.(mode);
        }],
        ["void scout", () => (window as any).__tw?.spawnCreature?.()],
        ["opening", () => (window as any).__tw?.orb?.play?.()],
        ["pilot ship", () => (window as any).__tw?.pilot?.enter?.(0)],
        ["frame", () => (window as any).__tw?.frameWorld?.()],
        ["raw render", () => (window as any).__tw?.diagBand?.({ post: false })],
        ["post fx", () => (window as any).__tw?.diagBand?.({ post: true })],
      ];
      for (const [label, fn] of actions) {
        ...
        b.onclick = () => {
          try {
            const result = fn();
            __twDebugLastAction = label + ": " + JSON.stringify(result);
          } catch (err: any) {
            __twDebugLastAction = label + " error: " + (err?.message || String(err));
          }
        };
    ...
    const __twFrame = (now: number) => {
      ...
      if (__twDebugEl && (frameCount % 15) === 0) {
        ...
        const fx = (window as any).__tw?.fxInfo?.() || { settle: 0, dust: 0, tethers: 0 };
        const drone = (window as any).__tw?.droneInfo?.() || null;
        const sent = (window as any).__tw?.sentinelInfo?.() || null;
        const outline = (window as any).__twOutline?.state;
        __twDebugEl.textContent = [
          "fps " + __twFps + "  phase ready  loopErr " + (!!(window as any).__twLoopError),
          "cam " + camera.position.x.toFixed(2) + "," + camera.position.y.toFixed(2) + "," + camera.position.z.toFixed(2) + "  jitter " + (window as any).__tw.jitter().toFixed(4),
          "blocks cols " + colMap.size + "  meshes " + meshesRef.current.length + "  voxel " + voxel.toFixed(3),
          "ledger W/S/B/V " + ledgerNow.world + "/" + ledgerNow.stockpile + "/" + ledgerNow.built + "/" + ledgerNow.void + "  base " + ledgerNow.baseline,
          "fx settle/dust/tether " + fx.settle + "/" + fx.dust + "/" + fx.tethers,
          "sentinel " + (sent ? sent.mode + " " + sent.state + " visible:" + sent.visible : "n/a"),
          "drone " + (drone ? drone.mode + " rig:" + drone.rigVisible + " light:" + Number(drone.light || 0).toFixed(2) : "n/a"),
          w0 ? "worker " + w0.vx + "," + w0.vz + " " + w0.mode + (w0.plan ? " plan:" + w0.plan.name : "") : "worker none",
          "outline " + (outline ? JSON.stringify(outline) : "n/a"),
          "last " + __twDebugLastAction,
        ].join("\n");
      }
    };
    (window as any).__tw = {
      orb: {
        say: orbSay, alert: orbAlert, mood: (m: any) => orbSetMood(m),
        show: (v: boolean) => orbShow(v), flush: orbFlush,
        play: startOpening, tutor: startTutorial,
      },
      pilot: { enter: (i?: number) => enterPilot(i), exit: (l?: boolean) => exitPilot(l), active: () => pilotActiveRef.current },
      opening: { play: startOpening, done: () => openingRef.done },
      setSentinelMode: (mode: "large" | "drone") => { setSentinelMode(mode === "drone" ? "drone" : "large"); return mode === "drone" ? "drone" : "large"; },
      setViewMode: (m: "cockpit" | "third") => { setViewMode(m === "third" ? "third" : "cockpit"); return m === "third" ? "third" : "cockpit"; },
      viewInfo: () => ({ view: viewModeRef.current, sentinelMode: sentinelModeRef.current, walking: walkingRef.current, sentinelVisible: sentinelGroup.visible, lift: viewModeRef.current === "third" ? voxel * 11 : voxel * 8, forward: thirdPersonRig.sentinelForward }),
      sentinelInfo: () => ({
        loaded: sentinelLoaded,
        state: sentinelAnimState,
        mode: sentinelModeRef.current,
        view: viewModeRef.current,
        walking: walkingRef.current,
        clips: Object.keys(sentinelActions),
        anchor: { ...droneAnchorRef },
        position: { x: sentinelGroup.position.x, y: sentinelGroup.position.y, z: sentinelGroup.position.z },
        thirdPerson: { enabled: viewModeRef.current === "third", forward: thirdPersonRig.sentinelForward },
        visible: sentinelGroup.visible,
      }),
      droneInfo: () => ({
        mode: sentinelModeRef.current,
        rigVisible: droneRig.visible,
        rig: { x: droneRig.position.x, y: droneRig.position.y, z: droneRig.position.z, rx: droneRig.rotation.x, rz: droneRig.rotation.z },
        thirdPerson: { enabled: true, forward: thirdPersonRig.droneForward, drop: thirdPersonRig.droneDrop },
        light: droneLight.intensity,
      }),
      settleTest: (layer?: string) => {
        const mesh = meshesRef.current.find((m: any) => (!layer || m.userData?.layer === layer) && (m.userData?.slotMap as Map<string, number> | undefined)?.size);
        if (!mesh) return { ok: false, reason: "no visible blocks" };
        const entry = Array.from((mesh.userData.slotMap as Map<string, number>).entries())[0] as [string, number];
        const [vx, vy, vz] = entry[0].split(",").map(Number);
        spawnSettleFx(mesh, entry[1], vx, vy, vz, mesh.userData.layer || "block", { x: vx - 4, y: vy + 2, z: vz - 4 });
        return { ok: true, layer: mesh.userData.layer, at: [vx, vy, vz], active: settleFx.length };
      },
      fxInfo: () => ({ settle: settleFx.length, dust: dustFx.length, tethers: relocationTethers.length }),
      frameWorld: () => {
        ...
        for (const m of meshesRef.current) {
          ...
          found = found || before !== box.isEmpty() || !box.isEmpty();
        }
        if (!found || box.isEmpty()) return { ok: false, reason: "no visible geometry" };
        const center = new THREE.Vector3();
        box.getCenter(center);
        const size = new THREE.Vector3();
        box.getSize(size);
        const footprint = Math.max(size.x, size.z, voxel * 16);
        orbit.target.set(center.x, Math.max(box.min.y, center.y * 0.35), center.z);
        camera.position.set(
          center.x + footprint * 0.9,
          box.max.y + footprint * 0.75,
          center.z + footprint * 0.9
        );
        ...
        return {
          ...
          cam: [Number(camera.position.x.toFixed(2)), Number(camera.position.y.toFixed(2)), Number(camera.position.z.toFixed(2))],
        };
      },
      state: () => ({
        voxel,
    ...
    // âââ Supplier Sentinel: MechaGolem GLB w/ rigged animations ââââââââââââââââ
    // Loaded from /sentinel.glb. The current Voxel Robots pack exports one
    // combined Unity take, so we split it into Idle/Run/Attack/Hit/Death from
    // the package's .meta frame ranges and alias Idle/Run into the older
    // Sitting/WalkSlow state-machine names.
    ...
    const bindSentinelClip = (clip: any, name = clip.name) => {
      const action = sentinelMixer.clipAction(clip);
      sentinelActions[name] = action;
      return action;
    };
    ...
    new GLTFLoader().load("/sentinel.glb", (gltf: any) => {
      ...
      root.traverse((obj: any) => {
        if (obj.isMesh) {
          ...
          // world's walls. BRDF (roughness/metalness) is matched EXACTLY to
          // the wall layer (addLayer stdMat) so the figure catches light
          // identically — no more off-tone reading under the same lighting.
      ...
      sentinelGroup.scale.setScalar(SENTINEL_TARGET_H / nativeH);

      sentinelMixer = new THREE.AnimationMixer(root);
      for (const clip of gltf.animations) {
        bindSentinelClip(clip);
      }
      const baseClip = gltf.animations.find((clip: any) => /MechaGolem_AllAnimExport|BaseLayer/i.test(clip.name)) || gltf.animations[0];
      if (baseClip && !sentinelActions.Idle && THREE.AnimationUtils?.subclip) {
        const fps = 30;
        [
          ["Idle", 0, 56],
          ["Run", 57, 77],
          ...
          ["Hit", 86, 115],
          ["Death", 116, 213],
        ].forEach(([name, start, end]: any) => {
          bindSentinelClip(THREE.AnimationUtils.subclip(baseClip, name, start, end, fps), name);
        });
      }
      if (sentinelActions.Idle && !sentinelActions.Sitting) sentinelActions.Sitting = sentinelActions.Idle;
      if (sentinelActions.Run && !sentinelActions.WalkSlow) sentinelActions.WalkSlow = sentinelActions.Run;
      sentinelLoaded = true;
      ...
      if (walkingRef.current) {
        ...
        if (!playSentinelClip("SitToStand", false)) {
          sentinelAnimState = "standing";
          playSentinelClip("WalkSlow", true) || playSentinelClip("Sitting", true);
        }
      } else {
        sentinelAnimState = "sitting";
        playSentinelClip("Sitting", true);
    ...
    const tickSentinel = (now: number) => {
      ...
      sentinelLastMs = now;
      const want = walkingRef.current && sentinelModeRef.current === "large";
      ...
      if (want && (sentinelAnimState === "sitting" || sentinelAnimState === "sittingDown")) {
        ...
        if (!playSentinelClip("StandToSit", false)) {
          sentinelAnimState = "sitting";
          playSentinelClip("Sitting", true);
        }
      }
    ...
    const renderSentinel = (_now: number) => {
      // View-mode framing: third-person pulls the sentinel closer (voxel*14,
      // was voxel*22) so it reads larger in frame; cockpit parks the body AT
      // the camera (forward 0) and hides it â you're sitting inside its head,
      // the cockpit HUD overlay provides the frame. Mutated every frame so the
      // body-ahead guard + info dump stay in sync.
      thirdPersonRig.sentinelForward = viewModeRef.current === "third" ? voxel * 14 : 0;
      const inCockpit = walkingRef.current && viewModeRef.current === "cockpit" && sentinelModeRef.current === "large";
      sentinelGroup.visible = !inCockpit;
      // In walk mode the sentinel becomes the player's avatar: position it
      // just ahead of the camera so large/drone control reads as third-person
      // while keeping the existing controller/raycast target stable.
      if (walkingRef.current) {
        if (sentinelModeRef.current === "drone" && droneAnchorRef.set) {
          ...
          sentinelYaw += (droneAnchorRef.yaw - sentinelYaw) * 0.18;
          sentinelGroup.rotation.y = sentinelYaw;
          return;
        }
        const forward = yawForward();
        const bodyX = camera.position.x + forward.x * thirdPersonRig.sentinelForward;
        const bodyZ = camera.position.z + forward.z * thirdPersonRig.sentinelForward;
        // Ground the sentinel at ITS OWN position, not the camera's. The avatar
        // renders ~sentinelForward voxels ahead; sampling ground at the camera
        // made the feet land on the camera's (often lower) column while the body
        // stood on higher terrain ahead — sinking the figure into cliffs. The
        // GLB origin is at the feet (verified), so position.y == ground == feet.
        const bodyGroundY = sampleGround(bodyX, bodyZ, camera.position.y - EYE_HEIGHT - camLiftFor(), playerR);
        const feetY = bodyGroundY;
        sentinelGroup.position.set(bodyX, feetY, bodyZ);
        // Share the sentinel's ground with the camera eye-clamp so the camera
        // rides the avatar's terrain instead of its own (see eye clamp below).
        sentinelFeetY = bodyGroundY;
        sentinelYaw += ((camera.rotation.y + Math.PI) - sentinelYaw) * 0.16;
        sentinelGroup.rotation.y = sentinelYaw;
        droneAnchorRef.x = bodyX;
        droneAnchorRef.y = feetY;
        droneAnchorRef.z = bodyZ;
        droneAnchorRef.yaw = sentinelYaw;
    ...
    const loop = () => {
      try {
        ...
        // Cinematic freeze: while simFrozen, skip continuous sim + (below) walk
        // physics so an anime.js scripted moment owns the camera/world cleanly.
        if (!simFrozen) {
        tickOrganic(now);
        ...
        if (!simFrozen && (fp.isLocked || __twSim.walk || (isMobileRef.current && walkingRef.current))) {
          ...
          tickDroneRig(now, dt, inLen, droneVert);
          ...
          if (isDrone) {
            // Drone: free 3D flight, but do not phase through geometry. If the
            // probe is blocked, restore the full previous position this frame.
            if (droneBlockedAt(newX, camera.position.y, newZ)) {
            ...
            // Sentinel body-ahead guard: the avatar is rendered ~sentinelForward
            // voxels in front of the camera. If that body voxel is solid, pull
            // the camera back so the figure stops at the wall instead of clipping.
            const fwd = yawForward();
            const bX = camera.position.x + fwd.x * thirdPersonRig.sentinelForward;
            const bZ = camera.position.z + fwd.z * thirdPersonRig.sentinelForward;
            const bFootWy = camera.position.y - EYE_HEIGHT - camLiftFor();
            if (sentinelBodyBlocked(bX, bZ, bFootWy)) {
          ...
          const camGroundY = sampleGround(camera.position.x, camera.position.z, camera.position.y - EYE_HEIGHT - camLiftFor(), playerR);
          // Ride the sentinel's ground when the avatar stands higher than the
          // camera (uphill / cliffs): otherwise the camera clamps to its own
          // lower column and ends up inside the sentinel's torso. Never drop
          // below the camera's own ground so the player still falls into holes
          // directly under the lens. sentinelFeetY is refreshed every frame by
          // renderSentinel (runs before this block in the same loop).
          const eyeGroundY = Math.max(camGroundY, sentinelFeetY);
          const eye = eyeGroundY + EYE_HEIGHT + camLiftFor();
          ...
          if (carryState) {
            ...
            // Advance hover-lift / snap (may clear carryState on snap done).
            tickCarryAnim();
            if ((frameCount % 2) === 0) {
              const target = getPlacementTarget();
              if (target) {
            ...
            // Idle hold: once lift settles (no active anim), float the block
            // at the hold position with a soft bob + faint sway for life.
            if (carryState && !carryAnim) {
              const hold = carryHoldPos();
              const bob = Math.sin(now * 0.005) * voxel * 0.05;
              carryBlockMesh.position.set(hold.x, hold.y + bob, hold.z);
              carryBlockMesh.rotation.y = Math.sin(now * 0.0015) * 0.08;
            }
          } else if (ghostMesh.visible) {
            ghostMesh.visible = false;
            (ghostMesh.material as any).color?.set?.(0xffffff);
            if (carryBlockMesh.visible) carryBlockMesh.visible = false;
          }
        } else {
          orbit.update();
...

        {
          ...
          let opacity = 0;
          if (carryState) {
            // Magnet tether follows the held block (always live), not a
            // possibly-hidden ghost target.
            endPos = { x: carryBlockMesh.position.x, y: carryBlockMesh.position.y, z: carryBlockMesh.position.z };
            opacity = 0.9;
          } else if (chargeState) {
            endPos = {
  ...
  const onLoadSelected = useCallback(async (overrideId?: string) => {
    ...
    try {
      ...
      if (!openingDoneRef.current) { openingDoneRef.current = true; startOpeningRef.current?.(); }
  ...
  return (
    ...
    >
      ...
      {debugMode && phase !== "ready" && (
        <div
          data-tw-debug-panel="1"
          style={{
            position: "fixed",
            left: "calc(env(safe-area-inset-left, 0px) + 10px)",
            top: "calc(env(safe-area-inset-top, 0px) + 10px)",
        ...
        >
          <div style={{ color: "#fff", fontWeight: 800, letterSpacing: 2, marginBottom: 8 }}>TINYWORLD DEBUG</div>
          {[
            "phase " + phase,
            "blocks " + blocks.toLocaleString(),
            "saved worlds " + worlds.filter((w) => w.hasSavedBlocks).length,
            "node " + (nodeInfo ? `${nodeInfo.city ?? "unknown"} (${nodeInfo.source})` : "pending"),
            "harness waiting for world build",
            loadNote ? "note " + loadNote : null,
          ].filter(Boolean).join("\n")}
        </div>
      )}
      <button
      ...
      {phase === "ready" && (
        <>
          {/* Orb companion subtitle (typewriter) */}
          {orbUi.visible && (
            <div className="absolute left-1/2 bottom-28 -translate-x-1/2 z-[1500] pointer-events-none flex flex-col items-center gap-1.5">
              <div className="px-5 py-2.5 rounded-lg bg-black/55 backdrop-blur-md border border-white/10 max-w-[80vw]">
                <span className="text-sm text-white/90 font-medium tracking-wide" style={{ fontFamily: "ui-monospace, SFMono-Regular, Menlo, monospace" }}>
                  {orbUi.text}<span className="animate-pulse text-white/50">_</span>
                </span>
              </div>
            </div>
          )}
          {/* Opening cinematic letterbox */}
          {openingUi.active && (
            <>
              <div className="absolute top-0 inset-x-0 h-[12vh] bg-black z-[1600] pointer-events-none" />
              <div className="absolute bottom-0 inset-x-0 h-[12vh] bg-black z-[1600] pointer-events-none" />
            </>
          )}
          {/* Pilot HUD */}
          {pilotUi.active && (
            <div className="absolute top-6 left-1/2 -translate-x-1/2 z-[1500] pointer-events-none">
              <div className="flex items-center gap-4 px-5 py-2 rounded-full bg-black/55 backdrop-blur-md border border-sky-400/30">
                <div className="flex items-center gap-1.5 text-sky-300">
                  <Ship className="w-4 h-4" />
                  <span className="text-xs font-mono">{Math.round(pilotUi.fuel)}/{pilotUi.maxFuel}</span>
                </div>
                <div className="text-xs font-mono text-white/70">{pilotUi.speed.toFixed(1)} v/s</div>
                <div className="text-xs font-mono text-white/70">{Math.round(pilotUi.altitude)} y</div>
                <div className="text-xs font-mono text-white/40">bank {Math.round(pilotUi.banking * 57)} deg</div>
                <div className="text-[10px] uppercase tracking-widest text-white/40">Esc to land</div>
              </div>
            </div>
          )}
        </>
            ...
            <div className="absolute inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[1000]">
              <div className="flex gap-8 p-12 flex-wrap justify-center max-w-5xl">
                ...
                {shipUi.count > 0 && (
                  ...
                  <button 
                    className="group flex flex-col items-center gap-2"
                    onClick={() => { enterPilotRef.current?.(0); setShowActions(false); }}>
                    <div className="w-24 h-24 rounded-full border-2 border-sky-400/20 flex items-center justify-center group-hover:border-sky-400/60 group-hover:bg-sky-500/10 transition-all">
                      <Ship className="w-10 h-10 text-sky-400/70 group-hover:text-sky-300" />
                    </div>
                    <span className="text-xs text-white/40 uppercase tracking-widest">Pilot Ship (fly manual)</span>
                  </button>
                )}
        ...
        <>
          ...
          <div className="absolute bottom-12 left-1/2 -translate-x-1/2 flex flex-col items-center gap-4 pointer-events-none z-40">
             <div className="flex items-center gap-8">
               ...
               <button
                 className="pointer-events-auto flex flex-col items-center gap-1 active:scale-90 transition-transform"
                 onClick={() => setSentinelMode((m) => m === "large" ? "drone" : "large")}
                 onTouchStart={(e) => { e.preventDefault(); e.stopPropagation(); setSentinelMode((m) => m === "large" ? "drone" : "large"); }}
               >
               ...
               {sentinelMode === "large" && (
                 <button
                   className="pointer-events-auto flex flex-col items-center gap-1 active:scale-90 transition-transform"
                   onClick={() => setViewMode((v) => v === "cockpit" ? "third" : "cockpit")}
                   onTouchStart={(e) => { e.preventDefault(); e.stopPropagation(); setViewMode((v) => v === "cockpit" ? "third" : "cockpit"); }}
                 >
                   <div className={`${isMobileRef.current ? "w-14 h-14" : "w-10 h-10"} rounded-full border-2 ${viewMode === "third" ? "border-amber-300/80 bg-amber-400/10" : "border-cyan-300/80 bg-cyan-400/10"} flex items-center justify-center backdrop-blur-md`}>
                     <span className={`${isMobileRef.current ? "text-[10px]" : "text-[9px]"} font-bold ${viewMode === "third" ? "text-amber-300" : "text-cyan-300"}`}>{viewMode === "third" ? "3RD" : "CPIT"}</span>
                   </div>
                   <span className="text-[9px] text-white/40 uppercase tracking-widest">{viewMode === "third" ? "3rd Person" : "Cockpit"}</span>
                 </button>
               )}
             ...
             {walking && sentinelMode === "large" && viewMode === "cockpit" && (
               <div className="pointer-events-none fixed inset-0 z-20 select-none">
                 <div className="absolute inset-0" style={{ boxShadow: "inset 0 0 30vw 5vw rgba(0,0,0,0.55)" }} />
                 {/* corner brackets */}
                 <div className="absolute" style={{ top: "5vh", left: "5vw", width: "10vw", height: "10vw", borderTop: "2px solid rgba(103,232,249,0.7)", borderLeft: "2px solid rgba(103,232,249,0.7)", filter: "drop-shadow(0 0 6px rgba(103,232,249,0.6))" }} />
                 <div className="absolute" style={{ top: "5vh", right: "5vw", width: "10vw", height: "10vw", borderTop: "2px solid rgba(103,232,249,0.7)", borderRight: "2px solid rgba(103,232,249,0.7)", filter: "drop-shadow(0 0 6px rgba(103,232,249,0.6))" }} />
                 <div className="absolute" style={{ bottom: "5vh", left: "5vw", width: "10vw", height: "10vw", borderBottom: "2px solid rgba(103,232,249,0.7)", borderLeft: "2px solid rgba(103,232,249,0.7)", filter: "drop-shadow(0 0 6px rgba(103,232,249,0.6))" }} />
                 <div className="absolute" style={{ bottom: "5vh", right: "5vw", width: "10vw", height: "10vw", borderBottom: "2px solid rgba(103,232,249,0.7)", borderRight: "2px solid rgba(103,232,249,0.7)", filter: "drop-shadow(0 0 6px rgba(103,232,249,0.6))" }} />
                 {/* center reticle */}
                 <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2" style={{ width: "clamp(40px,9vw,72px)", height: "clamp(40px,9vw,72px)", borderRadius: "9999px", border: "1.5px solid rgba(103,232,249,0.55)", boxShadow: "0 0 14px rgba(103,232,249,0.35), inset 0 0 14px rgba(103,232,249,0.2)" }}>
                   <div className="absolute left-1/2 top-0 -translate-x-1/2" style={{ width: "1.5px", height: "30%", background: "rgba(103,232,249,0.6)" }} />
                   <div className="absolute left-1/2 bottom-0 -translate-x-1/2" style={{ width: "1.5px", height: "30%", background: "rgba(103,232,249,0.6)" }} />
                   <div className="absolute top-1/2 left-0 -translate-y-1/2" style={{ height: "1.5px", width: "30%", background: "rgba(103,232,249,0.6)" }} />
                   <div className="absolute top-1/2 right-0 -translate-y-1/2" style={{ height: "1.5px", width: "30%", background: "rgba(103,232,249,0.6)" }} />
                 </div>
                 {/* telemetry */}
                 <div className="absolute" style={{ top: "7vh", left: "17vw", color: "rgba(103,232,249,0.85)", fontFamily: "monospace", fontSize: "clamp(9px,2.4vw,13px)", letterSpacing: "0.18em", textShadow: "0 0 8px rgba(103,232,249,0.55)", lineHeight: 1.7 }}>
                   <div>PILOT SYNCHRONIZATION&nbsp;&nbsp;98.43%</div>
                   <div>CONSCIOUSNESS ENABLED</div>
                   <div style={{ color: "rgba(255,95,189,0.85)" }}>SENTINEL LINK · NOMINAL</div>
                 </div>
                 <div className="absolute" style={{ bottom: "7vh", right: "17vw", color: "rgba(103,232,249,0.7)", fontFamily: "monospace", fontSize: "clamp(8px,2vw,11px)", letterSpacing: "0.2em", textAlign: "right", lineHeight: 1.7, textShadow: "0 0 8px rgba(103,232,249,0.5)" }}>
                   <div>MODE · COCKPIT</div>
                   <div>BULK · 4×4×4</div>
             ...
             {walking && sentinelMode === "drone" && (
               <div className="pointer-events-none fixed top-[3vh] left-1/2 -translate-x-1/2 z-20 px-4 py-1 rounded-full border border-cyan-300/40 bg-black/30 backdrop-blur-md" style={{ boxShadow: "0 0 18px rgba(103,232,249,0.3)" }}>
                 <span className="text-[10px] font-mono tracking-[0.25em] text-cyan-300">DRONE LINK ACTIVE</span>
               </div>
             )}
          </div>
        </>
